...
//
//  Description       :   This file is the resource provider for the cmdb_ci_app_server_jboss CMDB Class.  This code is executed
//                        when the servicenowcmdb_cmdb_ci_app_server_jboss keyword is used in a terraform script (*.tf) file
//
//                        This file is will need to be regenerated if the ServiceNow CMDB base CI Class
//                        "cmdb_ci" or of the cmdb_ci_app_server_jboss CI Class is modified.
///////////////////////////////////////////////////////////////////////////////////////////////////
import (
	"encoding/json"
	"fmt"
...
const CiNamecmdb_ci_app_server_jboss = "cmdb_ci_app_server_jboss"
...
// This is essentially the same as CmdbCiAppServerJbossGet but does not contain the "value", "link" and
...
//
type CmdbCiAppServerJbossPost struct {
	Result struct {
		Attributes struct {
...
// The generateprovidersource utility constructs this STRUCT from metadata pulled from ServiceNow.
//
type CmdbCiAppServerJbossGet struct {
	Result struct {
		Attributes struct {
			Container                   string `json:"container,omitempty"`
			TcpPort                     string `json:"tcp_port,omitempty"`
...
func ResourceCmdbCiAppServerJboss() *schema.Resource {
	return &schema.Resource{
		Create: createResourceCmdbCiAppServerJboss,
		Read:   readResourceCmdbCiAppServerJboss,
		Update: updateResourceCmdbCiAppServerJboss,
		Delete: deleteResourceCmdbCiAppServerJboss,
...
//
func createResourceCmdbCiAppServerJboss(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client) //Client Connection details
	// Use common function to update base attributes
	var ci CmdbCiAppServerJbossPost
	if err := copyFromTerraformToServiceNowCmdbCiAppServerJboss(resourceData, &ci); err != nil {
		return err
	}
	...
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_app_server_jboss + "?sysparm_fields=sys_id"
	...
	return readResourceCmdbCiAppServerJboss(resourceData, serviceNowClient)
}

//  Read routine - This function is called when Terraform wants to create a new CI in the ServiceNow CMDB
//
func readResourceCmdbCiAppServerJboss(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client)
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_app_server_jboss + "/" + resourceData.Id()
	var jsonData []byte
	...
	if err := copyFromServiceNowToTerraformCmdbCiAppServerJboss(resourceData, jsonData); err != nil {
		return err
	}
...
//
func updateResourceCmdbCiAppServerJboss(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	servicenowClient := serviceNowClient.(*Client)
...
//
func deleteResourceCmdbCiAppServerJboss(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	...
	if err := resourceData.Set("install_status", "retired"); err != nil {
		return fmt.Errorf("CmdbCiAppServerJbossfailed to set install_status field during destroy action %s", err)
	}
...
//
func copyFromTerraformToServiceNowCmdbCiAppServerJboss(resourceData *schema.ResourceData, ci *CmdbCiAppServerJbossPost) error {
...
//
func copyFromServiceNowToTerraformCmdbCiAppServerJboss(resourceData *schema.ResourceData, jsonData []byte) error {
	ci := CmdbCiAppServerJbossGet{}
