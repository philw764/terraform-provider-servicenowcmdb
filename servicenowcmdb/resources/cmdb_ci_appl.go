...
//
//  Description       :   This file is the resource provider for the cmdb_ci_appl CMDB Class.  This code is executed
//                        when the servicenowcmdb_cmdb_ci_appl keyword is used in a terraform script (*.tf) file
//
//                        This file is will need to be regenerated if the ServiceNow CMDB base CI Class
//                        "cmdb_ci" or of the cmdb_ci_appl CI Class is modified.
///////////////////////////////////////////////////////////////////////////////////////////////////
import (
	"encoding/json"
	"fmt"
...
const CiNamecmdb_ci_appl = "cmdb_ci_appl"
...
// This is essentially the same as CmdbCiApplGet but does not contain the "value", "link" and
...
func ResourceCmdbCiAppl() *schema.Resource {
	return &schema.Resource{
		Create: createResourceCmdbCiAppl,
		Read:   readResourceCmdbCiAppl,
		Update: updateResourceCmdbCiAppl,
		Delete: deleteResourceCmdbCiAppl,
...
//
func createResourceCmdbCiAppl(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client) //Client Connection details
	// Use common function to update base attributes
	var ci CmdbCiApplPost
	if err := copyFromTerraformToServiceNowCmdbCiAppl(resourceData, &ci); err != nil {
		return err
	}
	...
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_appl + "?sysparm_fields=sys_id"
	...
	return readResourceCmdbCiAppl(resourceData, serviceNowClient)
}

//  Read routine - This function is called when Terraform wants to create a new CI in the ServiceNow CMDB
//
func readResourceCmdbCiAppl(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client)
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_appl + "/" + resourceData.Id()
	var jsonData []byte
	...
	if err := copyFromServiceNowToTerraformCmdbCiAppl(resourceData, jsonData); err != nil {
		return err
	}
...
//
func updateResourceCmdbCiAppl(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	servicenowClient := serviceNowClient.(*Client)
...
//
func deleteResourceCmdbCiAppl(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	...
	if err := resourceData.Set("install_status", "retired"); err != nil {
		return fmt.Errorf("CmdbCiApplfailed to set install_status field during destroy action %s", err)
	}
...
//
func copyFromTerraformToServiceNowCmdbCiAppl(resourceData *schema.ResourceData, ci *CmdbCiApplPost) error {
...
//
func copyFromServiceNowToTerraformCmdbCiAppl(resourceData *schema.ResourceData, jsonData []byte) error {
	ci := CmdbCiApplGet{}
