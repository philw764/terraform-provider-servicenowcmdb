...
//
//  Description       :   This file is the resource provider for the cmdb_ci_app_simulation CMDB Class.  This code is executed
//                        when the servicenowcmdb_cmdb_ci_app_simulation keyword is used in a terraform script (*.tf) file
//
//                        This file is will need to be regenerated if the ServiceNow CMDB base CI Class
//                        "cmdb_ci" or of the cmdb_ci_app_simulation CI Class is modified.
///////////////////////////////////////////////////////////////////////////////////////////////////
import (
	"encoding/json"
	"fmt"
...
const CiNamecmdb_ci_app_simulation = "cmdb_ci_app_simulation"
...
// This is essentially the same as CmdbCiAppSimulationGet but does not contain the "value", "link" and
...
//
type CmdbCiAppSimulationPost struct {
	Result struct {
		Attributes struct {
...
// The generateprovidersource utility constructs this STRUCT from metadata pulled from ServiceNow.
//
type CmdbCiAppSimulationGet struct {
	Result struct {
		Attributes struct {
			TcpPort                     string `json:"tcp_port,omitempty"`
			SkipSync                    string `json:"skip_sync,omitempty"`
...
func ResourceCmdbCiAppSimulation() *schema.Resource {
	return &schema.Resource{
		Create: createResourceCmdbCiAppSimulation,
		Read:   readResourceCmdbCiAppSimulation,
		Update: updateResourceCmdbCiAppSimulation,
		Delete: deleteResourceCmdbCiAppSimulation,
...
//
func createResourceCmdbCiAppSimulation(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client) //Client Connection details
	// Use common function to update base attributes
	var ci CmdbCiAppSimulationPost
	if err := copyFromTerraformToServiceNowCmdbCiAppSimulation(resourceData, &ci); err != nil {
		return err
	}
	...
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_app_simulation + "?sysparm_fields=sys_id"
	...
	return readResourceCmdbCiAppSimulation(resourceData, serviceNowClient)
}

//  Read routine - This function is called when Terraform wants to create a new CI in the ServiceNow CMDB
//
func readResourceCmdbCiAppSimulation(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client)
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_app_simulation + "/" + resourceData.Id()
	var jsonData []byte
	...
	if err := copyFromServiceNowToTerraformCmdbCiAppSimulation(resourceData, jsonData); err != nil {
		return err
	}
...
//
func updateResourceCmdbCiAppSimulation(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	servicenowClient := serviceNowClient.(*Client)
...
//
func deleteResourceCmdbCiAppSimulation(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	...
	if err := resourceData.Set("install_status", "retired"); err != nil {
		return fmt.Errorf("CmdbCiAppSimulationfailed to set install_status field during destroy action %s", err)
	}
...
//
func copyFromTerraformToServiceNowCmdbCiAppSimulation(resourceData *schema.ResourceData, ci *CmdbCiAppSimulationPost) error {
...
//
func copyFromServiceNowToTerraformCmdbCiAppSimulation(resourceData *schema.ResourceData, jsonData []byte) error {
	ci := CmdbCiAppSimulationGet{}
