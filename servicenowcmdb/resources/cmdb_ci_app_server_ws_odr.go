...
//
//  Description       :   This file is the resource provider for the cmdb_ci_app_server_ws_odr CMDB Class.  This code is executed
//                        when the servicenowcmdb_cmdb_ci_app_server_ws_odr keyword is used in a terraform script (*.tf) file
//
//                        This file is will need to be regenerated if the ServiceNow CMDB base CI Class
//                        "cmdb_ci" or of the cmdb_ci_app_server_ws_odr CI Class is modified.
///////////////////////////////////////////////////////////////////////////////////////////////////
import (
	"encoding/json"
	"fmt"
...
const CiNamecmdb_ci_app_server_ws_odr = "cmdb_ci_app_server_ws_odr"
...
// This is essentially the same as CmdbCiAppServerWsOdrGet but does not contain the "value", "link" and
...
//
type CmdbCiAppServerWsOdrPost struct {
	Result struct {
		Attributes struct {
...
// The generateprovidersource utility constructs this STRUCT from metadata pulled from ServiceNow.
//
type CmdbCiAppServerWsOdrGet struct {
	Result struct {
		Attributes struct {
			Container                   string `json:"container,omitempty"`
			TcpPort                     string `json:"tcp_port,omitempty"`
...
func ResourceCmdbCiAppServerWsOdr() *schema.Resource {
	return &schema.Resource{
		Create: createResourceCmdbCiAppServerWsOdr,
		Read:   readResourceCmdbCiAppServerWsOdr,
		Update: updateResourceCmdbCiAppServerWsOdr,
		Delete: deleteResourceCmdbCiAppServerWsOdr,
...
//
func createResourceCmdbCiAppServerWsOdr(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client) //Client Connection details
	// Use common function to update base attributes
	var ci CmdbCiAppServerWsOdrPost
	if err := copyFromTerraformToServiceNowCmdbCiAppServerWsOdr(resourceData, &ci); err != nil {
		return err
	}
	...
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_app_server_ws_odr + "?sysparm_fields=sys_id"
	...
	return readResourceCmdbCiAppServerWsOdr(resourceData, serviceNowClient)
}

//  Read routine - This function is called when Terraform wants to create a new CI in the ServiceNow CMDB
//
func readResourceCmdbCiAppServerWsOdr(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client)
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_app_server_ws_odr + "/" + resourceData.Id()
	var jsonData []byte
	...
	if err := copyFromServiceNowToTerraformCmdbCiAppServerWsOdr(resourceData, jsonData); err != nil {
		return err
	}
...
//
func updateResourceCmdbCiAppServerWsOdr(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	servicenowClient := serviceNowClient.(*Client)
...
//
func deleteResourceCmdbCiAppServerWsOdr(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	...
	if err := resourceData.Set("install_status", "retired"); err != nil {
		return fmt.Errorf("CmdbCiAppServerWsOdrfailed to set install_status field during destroy action %s", err)
	}
...
//
func copyFromTerraformToServiceNowCmdbCiAppServerWsOdr(resourceData *schema.ResourceData, ci *CmdbCiAppServerWsOdrPost) error {
...
//
func copyFromServiceNowToTerraformCmdbCiAppServerWsOdr(resourceData *schema.ResourceData, jsonData []byte) error {
	ci := CmdbCiAppServerWsOdrGet{}
