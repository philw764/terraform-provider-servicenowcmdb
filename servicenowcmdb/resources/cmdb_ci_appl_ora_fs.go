...
//
//  Description       :   This file is the resource provider for the cmdb_ci_appl_ora_fs CMDB Class.  This code is executed
//                        when the servicenowcmdb_cmdb_ci_appl_ora_fs keyword is used in a terraform script (*.tf) file
//
//                        This file is will need to be regenerated if the ServiceNow CMDB base CI Class
//                        "cmdb_ci" or of the cmdb_ci_appl_ora_fs CI Class is modified.
///////////////////////////////////////////////////////////////////////////////////////////////////
import (
	"encoding/json"
	"fmt"
...
const CiNamecmdb_ci_appl_ora_fs = "cmdb_ci_appl_ora_fs"
...
// This is essentially the same as CmdbCiApplOraFsGet but does not contain the "value", "link" and
...
//
type CmdbCiApplOraFsPost struct {
	Result struct {
		Attributes struct {
...
// The generateprovidersource utility constructs this STRUCT from metadata pulled from ServiceNow.
//
type CmdbCiApplOraFsGet struct {
	Result struct {
		Attributes struct {
			TcpPort                     string `json:"tcp_port,omitempty"`
			SkipSync                    string `json:"skip_sync,omitempty"`
...
func ResourceCmdbCiApplOraFs() *schema.Resource {
	return &schema.Resource{
		Create: createResourceCmdbCiApplOraFs,
		Read:   readResourceCmdbCiApplOraFs,
		Update: updateResourceCmdbCiApplOraFs,
		Delete: deleteResourceCmdbCiApplOraFs,
...
//
func createResourceCmdbCiApplOraFs(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client) //Client Connection details
	// Use common function to update base attributes
	var ci CmdbCiApplOraFsPost
	if err := copyFromTerraformToServiceNowCmdbCiApplOraFs(resourceData, &ci); err != nil {
		return err
	}
	...
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_appl_ora_fs + "?sysparm_fields=sys_id"
	...
	return readResourceCmdbCiApplOraFs(resourceData, serviceNowClient)
}

//  Read routine - This function is called when Terraform wants to create a new CI in the ServiceNow CMDB
//
func readResourceCmdbCiApplOraFs(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client)
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_appl_ora_fs + "/" + resourceData.Id()
	var jsonData []byte
	...
	if err := copyFromServiceNowToTerraformCmdbCiApplOraFs(resourceData, jsonData); err != nil {
		return err
	}
...
//
func updateResourceCmdbCiApplOraFs(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	servicenowClient := serviceNowClient.(*Client)
...
//
func deleteResourceCmdbCiApplOraFs(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	...
	if err := resourceData.Set("install_status", "retired"); err != nil {
		return fmt.Errorf("CmdbCiApplOraFsfailed to set install_status field during destroy action %s", err)
	}
...
//
func copyFromTerraformToServiceNowCmdbCiApplOraFs(resourceData *schema.ResourceData, ci *CmdbCiApplOraFsPost) error {
...
//
func copyFromServiceNowToTerraformCmdbCiApplOraFs(resourceData *schema.ResourceData, jsonData []byte) error {
	ci := CmdbCiApplOraFsGet{}
