...
//
//  Description       :   This file is the resource provider for the cmdb_ci_win_cluster_node CMDB Class.  This code is executed
//                        when the servicenowcmdb_cmdb_ci_win_cluster_node keyword is used in a terraform script (*.tf) file
//
//                        This file is will need to be regenerated if the ServiceNow CMDB base CI Class
//                        "cmdb_ci" or of the cmdb_ci_win_cluster_node CI Class is modified.
///////////////////////////////////////////////////////////////////////////////////////////////////
import (
	"encoding/json"
	"fmt"
...
const CiNamecmdb_ci_win_cluster_node = "cmdb_ci_win_cluster_node"
...
// This is essentially the same as CmdbCiWinClusterNodeGet but does not contain the "value", "link" and
...
//
type CmdbCiWinClusterNodePost struct {
	Result struct {
		Attributes struct {
...
// The generateprovidersource utility constructs this STRUCT from metadata pulled from ServiceNow.
//
type CmdbCiWinClusterNodeGet struct {
	Result struct {
		Attributes struct {
			Cluster struct {
...
func ResourceCmdbCiWinClusterNode() *schema.Resource {
	return &schema.Resource{
		Create: createResourceCmdbCiWinClusterNode,
		Read:   readResourceCmdbCiWinClusterNode,
		Update: updateResourceCmdbCiWinClusterNode,
		Delete: deleteResourceCmdbCiWinClusterNode,
...
//
func createResourceCmdbCiWinClusterNode(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client) //Client Connection details
	// Use common function to update base attributes
	var ci CmdbCiWinClusterNodePost
	if err := copyFromTerraformToServiceNowCmdbCiWinClusterNode(resourceData, &ci); err != nil {
		return err
	}
	...
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_win_cluster_node + "?sysparm_fields=sys_id"
	...
	return readResourceCmdbCiWinClusterNode(resourceData, serviceNowClient)
}

//  Read routine - This function is called when Terraform wants to create a new CI in the ServiceNow CMDB
//
func readResourceCmdbCiWinClusterNode(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client)
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_win_cluster_node + "/" + resourceData.Id()
	var jsonData []byte
	...
	if err := copyFromServiceNowToTerraformCmdbCiWinClusterNode(resourceData, jsonData); err != nil {
		return err
	}
...
//
func updateResourceCmdbCiWinClusterNode(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	servicenowClient := serviceNowClient.(*Client)
...
//
func deleteResourceCmdbCiWinClusterNode(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	...
	if err := resourceData.Set("install_status", "retired"); err != nil {
		return fmt.Errorf("CmdbCiWinClusterNodefailed to set install_status field during destroy action %s", err)
	}
...
//
func copyFromTerraformToServiceNowCmdbCiWinClusterNode(resourceData *schema.ResourceData, ci *CmdbCiWinClusterNodePost) error {

	attrs := &ci.Result.Attributes
	attrs.Cluster = resourceData.Get("cluster.value").(string)
	attrs.SkipSync = resourceData.Get("skip_sync").(string)
	attrs.OperationalStatus = resourceData.Get("operational_status").(string)
	attrs.Caption = resourceData.Get("caption").(string)
	...
	attrs.NodeStatus = resourceData.Get("node_status").(string)
	...
	attrs.NodeState = resourceData.Get("node_state").(string)
	...
	attrs.Server = resourceData.Get("server.value").(string)
...
//
func copyFromServiceNowToTerraformCmdbCiWinClusterNode(resourceData *schema.ResourceData, jsonData []byte) error {
	ci := CmdbCiWinClusterNodeGet{}
	...
	_ = resourceData.Set("node_status", attrs.NodeStatus)
	...
	_ = resourceData.Set("node_state", attrs.NodeState)
	...
	_ = resourceData.Set("server", StructToMap(attrs.Server))
