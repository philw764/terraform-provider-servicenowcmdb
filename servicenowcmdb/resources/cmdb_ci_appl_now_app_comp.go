...
//
//  Description       :   This file is the resource provider for the cmdb_ci_appl_now_app_comp CMDB Class.  This code is executed
//                        when the servicenowcmdb_cmdb_ci_appl_now_app_comp keyword is used in a terraform script (*.tf) file
//
//                        This file is will need to be regenerated if the ServiceNow CMDB base CI Class
//                        "cmdb_ci" or of the cmdb_ci_appl_now_app_comp CI Class is modified.
///////////////////////////////////////////////////////////////////////////////////////////////////
import (
	"encoding/json"
	"fmt"
...
const CiNamecmdb_ci_appl_now_app_comp = "cmdb_ci_appl_now_app_comp"
...
// This is essentially the same as CmdbCiApplNowAppCompGet but does not contain the "value", "link" and
...
//
type CmdbCiApplNowAppCompPost struct {
	Result struct {
		Attributes struct {
...
// The generateprovidersource utility constructs this STRUCT from metadata pulled from ServiceNow.
//
type CmdbCiApplNowAppCompGet struct {
	Result struct {
		Attributes struct {
			TcpPort                     string `json:"tcp_port,omitempty"`
			SkipSync                    string `json:"skip_sync,omitempty"`
...
func ResourceCmdbCiApplNowAppComp() *schema.Resource {
	return &schema.Resource{
		Create: createResourceCmdbCiApplNowAppComp,
		Read:   readResourceCmdbCiApplNowAppComp,
		Update: updateResourceCmdbCiApplNowAppComp,
		Delete: deleteResourceCmdbCiApplNowAppComp,
...
//
func createResourceCmdbCiApplNowAppComp(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client) //Client Connection details
	// Use common function to update base attributes
	var ci CmdbCiApplNowAppCompPost
	if err := copyFromTerraformToServiceNowCmdbCiApplNowAppComp(resourceData, &ci); err != nil {
		return err
	}
	...
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_appl_now_app_comp + "?sysparm_fields=sys_id"
	...
	return readResourceCmdbCiApplNowAppComp(resourceData, serviceNowClient)
}

//  Read routine - This function is called when Terraform wants to create a new CI in the ServiceNow CMDB
//
func readResourceCmdbCiApplNowAppComp(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client)
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_appl_now_app_comp + "/" + resourceData.Id()
	var jsonData []byte
	...
	if err := copyFromServiceNowToTerraformCmdbCiApplNowAppComp(resourceData, jsonData); err != nil {
		return err
	}
...
//
func updateResourceCmdbCiApplNowAppComp(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	servicenowClient := serviceNowClient.(*Client)
...
//
func deleteResourceCmdbCiApplNowAppComp(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	...
	if err := resourceData.Set("install_status", "retired"); err != nil {
		return fmt.Errorf("CmdbCiApplNowAppCompfailed to set install_status field during destroy action %s", err)
	}
...
//
func copyFromTerraformToServiceNowCmdbCiApplNowAppComp(resourceData *schema.ResourceData, ci *CmdbCiApplNowAppCompPost) error {
...
//
func copyFromServiceNowToTerraformCmdbCiApplNowAppComp(resourceData *schema.ResourceData, jsonData []byte) error {
	ci := CmdbCiApplNowAppCompGet{}
