...
//
//  Description       :   This file is the resource provider for the cmdb_ci_fc_export CMDB Class.  This code is executed
//                        when the servicenowcmdb_cmdb_ci_fc_export keyword is used in a terraform script (*.tf) file
//
//                        This file is will need to be regenerated if the ServiceNow CMDB base CI Class
//                        "cmdb_ci" or of the cmdb_ci_fc_export CI Class is modified.
///////////////////////////////////////////////////////////////////////////////////////////////////
import (
	"encoding/json"
	"fmt"
...
const CiNamecmdb_ci_fc_export = "cmdb_ci_fc_export"
...
// This is essentially the same as CmdbCiFcExportGet but does not contain the "value", "link" and
...
//
type CmdbCiFcExportPost struct {
	Result struct {
		Attributes struct {
...
func ResourceCmdbCiFcExport() *schema.Resource {
	return &schema.Resource{
		Create: createResourceCmdbCiFcExport,
		Read:   readResourceCmdbCiFcExport,
		Update: updateResourceCmdbCiFcExport,
		Delete: deleteResourceCmdbCiFcExport,
...
//
func createResourceCmdbCiFcExport(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client) //Client Connection details
	// Use common function to update base attributes
	var ci CmdbCiFcExportPost
	if err := copyFromTerraformToServiceNowCmdbCiFcExport(resourceData, &ci); err != nil {
		return err
	}
	...
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_fc_export + "?sysparm_fields=sys_id"
	...
	return readResourceCmdbCiFcExport(resourceData, serviceNowClient)
}

//  Read routine - This function is called when Terraform wants to create a new CI in the ServiceNow CMDB
//
func readResourceCmdbCiFcExport(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client)
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_fc_export + "/" + resourceData.Id()
	var jsonData []byte
	...
	if err := copyFromServiceNowToTerraformCmdbCiFcExport(resourceData, jsonData); err != nil {
		return err
	}
...
//
func updateResourceCmdbCiFcExport(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	servicenowClient := serviceNowClient.(*Client)
...
//
func deleteResourceCmdbCiFcExport(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	...
	if err := resourceData.Set("install_status", "retired"); err != nil {
		return fmt.Errorf("CmdbCiFcExportfailed to set install_status field during destroy action %s", err)
	}
...
//
func copyFromTerraformToServiceNowCmdbCiFcExport(resourceData *schema.ResourceData, ci *CmdbCiFcExportPost) error {
	...
	attrs.InitiatorWwpn = resourceData.Get("initiator_wwpn").(string)
...
//
func copyFromServiceNowToTerraformCmdbCiFcExport(resourceData *schema.ResourceData, jsonData []byte) error {
	ci := CmdbCiFcExportGet{}
	...
	_ = resourceData.Set("initiator_wwpn", attrs.InitiatorWwpn)
