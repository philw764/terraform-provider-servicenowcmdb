...
//
//  Description       :   This file is the resource provider for the cmdb_ci_appl_ora_gg_extract CMDB Class.  This code is executed
//                        when the servicenowcmdb_cmdb_ci_appl_ora_gg_extract keyword is used in a terraform script (*.tf) file
//
//                        This file is will need to be regenerated if the ServiceNow CMDB base CI Class
//                        "cmdb_ci" or of the cmdb_ci_appl_ora_gg_extract CI Class is modified.
///////////////////////////////////////////////////////////////////////////////////////////////////
import (
	"encoding/json"
	"fmt"
...
const CiNamecmdb_ci_appl_ora_gg_extract = "cmdb_ci_appl_ora_gg_extract"
...
// This is essentially the same as CmdbCiApplOraGgExtractGet but does not contain the "value", "link" and
...
//
type CmdbCiApplOraGgExtractPost struct {
	Result struct {
		Attributes struct {
...
// The generateprovidersource utility constructs this STRUCT from metadata pulled from ServiceNow.
//
type CmdbCiApplOraGgExtractGet struct {
	Result struct {
		Attributes struct {
			TcpPort                     string `json:"tcp_port,omitempty"`
			SkipSync                    string `json:"skip_sync,omitempty"`
...
func ResourceCmdbCiApplOraGgExtract() *schema.Resource {
	return &schema.Resource{
		Create: createResourceCmdbCiApplOraGgExtract,
		Read:   readResourceCmdbCiApplOraGgExtract,
		Update: updateResourceCmdbCiApplOraGgExtract,
		Delete: deleteResourceCmdbCiApplOraGgExtract,
...
//
func createResourceCmdbCiApplOraGgExtract(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client) //Client Connection details
	// Use common function to update base attributes
	var ci CmdbCiApplOraGgExtractPost
	if err := copyFromTerraformToServiceNowCmdbCiApplOraGgExtract(resourceData, &ci); err != nil {
		return err
	}
	...
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_appl_ora_gg_extract + "?sysparm_fields=sys_id"
	...
	return readResourceCmdbCiApplOraGgExtract(resourceData, serviceNowClient)
}

//  Read routine - This function is called when Terraform wants to create a new CI in the ServiceNow CMDB
//
func readResourceCmdbCiApplOraGgExtract(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client)
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_appl_ora_gg_extract + "/" + resourceData.Id()
	var jsonData []byte
	...
	if err := copyFromServiceNowToTerraformCmdbCiApplOraGgExtract(resourceData, jsonData); err != nil {
		return err
	}
...
//
func updateResourceCmdbCiApplOraGgExtract(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	servicenowClient := serviceNowClient.(*Client)
...
//
func deleteResourceCmdbCiApplOraGgExtract(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	...
	if err := resourceData.Set("install_status", "retired"); err != nil {
		return fmt.Errorf("CmdbCiApplOraGgExtractfailed to set install_status field during destroy action %s", err)
	}
...
//
func copyFromTerraformToServiceNowCmdbCiApplOraGgExtract(resourceData *schema.ResourceData, ci *CmdbCiApplOraGgExtractPost) error {
...
//
func copyFromServiceNowToTerraformCmdbCiApplOraGgExtract(resourceData *schema.ResourceData, jsonData []byte) error {
	ci := CmdbCiApplOraGgExtractGet{}
