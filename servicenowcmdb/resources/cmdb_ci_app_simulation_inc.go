...
//
//  Description       :   This file is the resource provider for the cmdb_ci_app_simulation_inc CMDB Class.  This code is executed
//                        when the servicenowcmdb_cmdb_ci_app_simulation_inc keyword is used in a terraform script (*.tf) file
//
//                        This file is will need to be regenerated if the ServiceNow CMDB base CI Class
//                        "cmdb_ci" or of the cmdb_ci_app_simulation_inc CI Class is modified.
///////////////////////////////////////////////////////////////////////////////////////////////////
import (
	"encoding/json"
	"fmt"
...
const CiNamecmdb_ci_app_simulation_inc = "cmdb_ci_app_simulation_inc"
...
// This is essentially the same as CmdbCiAppSimulationIncGet but does not contain the "value", "link" and
...
//
type CmdbCiAppSimulationIncPost struct {
	Result struct {
		Attributes struct {
...
// The generateprovidersource utility constructs this STRUCT from metadata pulled from ServiceNow.
//
type CmdbCiAppSimulationIncGet struct {
	Result struct {
		Attributes struct {
			TcpPort                     string `json:"tcp_port,omitempty"`
			SkipSync                    string `json:"skip_sync,omitempty"`
...
func ResourceCmdbCiAppSimulationInc() *schema.Resource {
	return &schema.Resource{
		Create: createResourceCmdbCiAppSimulationInc,
		Read:   readResourceCmdbCiAppSimulationInc,
		Update: updateResourceCmdbCiAppSimulationInc,
		Delete: deleteResourceCmdbCiAppSimulationInc,
...
//
func createResourceCmdbCiAppSimulationInc(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client) //Client Connection details
	// Use common function to update base attributes
	var ci CmdbCiAppSimulationIncPost
	if err := copyFromTerraformToServiceNowCmdbCiAppSimulationInc(resourceData, &ci); err != nil {
		return err
	}
	...
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_app_simulation_inc + "?sysparm_fields=sys_id"
	...
	return readResourceCmdbCiAppSimulationInc(resourceData, serviceNowClient)
}

//  Read routine - This function is called when Terraform wants to create a new CI in the ServiceNow CMDB
//
func readResourceCmdbCiAppSimulationInc(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client)
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_app_simulation_inc + "/" + resourceData.Id()
	var jsonData []byte
	...
	if err := copyFromServiceNowToTerraformCmdbCiAppSimulationInc(resourceData, jsonData); err != nil {
		return err
	}
...
//
func updateResourceCmdbCiAppSimulationInc(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	servicenowClient := serviceNowClient.(*Client)
...
//
func deleteResourceCmdbCiAppSimulationInc(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	...
	if err := resourceData.Set("install_status", "retired"); err != nil {
		return fmt.Errorf("CmdbCiAppSimulationIncfailed to set install_status field during destroy action %s", err)
	}
...
//
func copyFromTerraformToServiceNowCmdbCiAppSimulationInc(resourceData *schema.ResourceData, ci *CmdbCiAppSimulationIncPost) error {
...
//
func copyFromServiceNowToTerraformCmdbCiAppSimulationInc(resourceData *schema.ResourceData, jsonData []byte) error {
	ci := CmdbCiAppSimulationIncGet{}
