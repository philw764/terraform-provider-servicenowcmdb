...
//
//  Description       :   This file is the resource provider for the cmdb_ci_appl_ora_report CMDB Class.  This code is executed
//                        when the servicenowcmdb_cmdb_ci_appl_ora_report keyword is used in a terraform script (*.tf) file
//
//                        This file is will need to be regenerated if the ServiceNow CMDB base CI Class
//                        "cmdb_ci" or of the cmdb_ci_appl_ora_report CI Class is modified.
///////////////////////////////////////////////////////////////////////////////////////////////////
import (
	"encoding/json"
	"fmt"
...
const CiNamecmdb_ci_appl_ora_report = "cmdb_ci_appl_ora_report"
...
// This is essentially the same as CmdbCiApplOraReportGet but does not contain the "value", "link" and
...
//
type CmdbCiApplOraReportPost struct {
	Result struct {
		Attributes struct {
...
// The generateprovidersource utility constructs this STRUCT from metadata pulled from ServiceNow.
//
type CmdbCiApplOraReportGet struct {
	Result struct {
		Attributes struct {
			TcpPort                     string `json:"tcp_port,omitempty"`
			SkipSync                    string `json:"skip_sync,omitempty"`
...
func ResourceCmdbCiApplOraReport() *schema.Resource {
	return &schema.Resource{
		Create: createResourceCmdbCiApplOraReport,
		Read:   readResourceCmdbCiApplOraReport,
		Update: updateResourceCmdbCiApplOraReport,
		Delete: deleteResourceCmdbCiApplOraReport,
...
//
func createResourceCmdbCiApplOraReport(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client) //Client Connection details
	// Use common function to update base attributes
	var ci CmdbCiApplOraReportPost
	if err := copyFromTerraformToServiceNowCmdbCiApplOraReport(resourceData, &ci); err != nil {
		return err
	}
	...
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_appl_ora_report + "?sysparm_fields=sys_id"
	...
	return readResourceCmdbCiApplOraReport(resourceData, serviceNowClient)
}

//  Read routine - This function is called when Terraform wants to create a new CI in the ServiceNow CMDB
//
func readResourceCmdbCiApplOraReport(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client)
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_appl_ora_report + "/" + resourceData.Id()
	var jsonData []byte
	...
	if err := copyFromServiceNowToTerraformCmdbCiApplOraReport(resourceData, jsonData); err != nil {
		return err
	}
...
//
func updateResourceCmdbCiApplOraReport(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	servicenowClient := serviceNowClient.(*Client)
...
//
func deleteResourceCmdbCiApplOraReport(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	...
	if err := resourceData.Set("install_status", "retired"); err != nil {
		return fmt.Errorf("CmdbCiApplOraReportfailed to set install_status field during destroy action %s", err)
	}
...
//
func copyFromTerraformToServiceNowCmdbCiApplOraReport(resourceData *schema.ResourceData, ci *CmdbCiApplOraReportPost) error {
...
//
func copyFromServiceNowToTerraformCmdbCiApplOraReport(resourceData *schema.ResourceData, jsonData []byte) error {
	ci := CmdbCiApplOraReportGet{}
