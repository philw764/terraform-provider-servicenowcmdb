...
//
//  Description       :   This file is the resource provider for the cmdb_ci_appl_hp_sm_kb CMDB Class.  This code is executed
//                        when the servicenowcmdb_cmdb_ci_appl_hp_sm_kb keyword is used in a terraform script (*.tf) file
//
//                        This file is will need to be regenerated if the ServiceNow CMDB base CI Class
//                        "cmdb_ci" or of the cmdb_ci_appl_hp_sm_kb CI Class is modified.
///////////////////////////////////////////////////////////////////////////////////////////////////
import (
	"encoding/json"
	"fmt"
...
const CiNamecmdb_ci_appl_hp_sm_kb = "cmdb_ci_appl_hp_sm_kb"
...
// This is essentially the same as CmdbCiApplHpSmKbGet but does not contain the "value", "link" and
...
//
type CmdbCiApplHpSmKbPost struct {
	Result struct {
		Attributes struct {
...
// The generateprovidersource utility constructs this STRUCT from metadata pulled from ServiceNow.
//
type CmdbCiApplHpSmKbGet struct {
	Result struct {
		Attributes struct {
			TcpPort                     string `json:"tcp_port,omitempty"`
			SkipSync                    string `json:"skip_sync,omitempty"`
...
func ResourceCmdbCiApplHpSmKb() *schema.Resource {
	return &schema.Resource{
		Create: createResourceCmdbCiApplHpSmKb,
		Read:   readResourceCmdbCiApplHpSmKb,
		Update: updateResourceCmdbCiApplHpSmKb,
		Delete: deleteResourceCmdbCiApplHpSmKb,
...
//
func createResourceCmdbCiApplHpSmKb(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client) //Client Connection details
	// Use common function to update base attributes
	var ci CmdbCiApplHpSmKbPost
	if err := copyFromTerraformToServiceNowCmdbCiApplHpSmKb(resourceData, &ci); err != nil {
		return err
	}
	...
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_appl_hp_sm_kb + "?sysparm_fields=sys_id"
	...
	return readResourceCmdbCiApplHpSmKb(resourceData, serviceNowClient)
}

//  Read routine - This function is called when Terraform wants to create a new CI in the ServiceNow CMDB
//
func readResourceCmdbCiApplHpSmKb(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client)
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_appl_hp_sm_kb + "/" + resourceData.Id()
	var jsonData []byte
	...
	if err := copyFromServiceNowToTerraformCmdbCiApplHpSmKb(resourceData, jsonData); err != nil {
		return err
	}
...
//
func updateResourceCmdbCiApplHpSmKb(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	servicenowClient := serviceNowClient.(*Client)
...
//
func deleteResourceCmdbCiApplHpSmKb(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	...
	if err := resourceData.Set("install_status", "retired"); err != nil {
		return fmt.Errorf("CmdbCiApplHpSmKbfailed to set install_status field during destroy action %s", err)
	}
...
//
func copyFromTerraformToServiceNowCmdbCiApplHpSmKb(resourceData *schema.ResourceData, ci *CmdbCiApplHpSmKbPost) error {
...
//
func copyFromServiceNowToTerraformCmdbCiApplHpSmKb(resourceData *schema.ResourceData, jsonData []byte) error {
	ci := CmdbCiApplHpSmKbGet{}
