...
//
//  Description       :   This file is the resource provider for the cmdb_ci_ucs_rack_unit CMDB Class.  This code is executed
//                        when the servicenowcmdb_cmdb_ci_ucs_rack_unit keyword is used in a terraform script (*.tf) file
//
//                        This file is will need to be regenerated if the ServiceNow CMDB base CI Class
//                        "cmdb_ci" or of the cmdb_ci_ucs_rack_unit CI Class is modified.
///////////////////////////////////////////////////////////////////////////////////////////////////
import (
	"encoding/json"
	"fmt"
...
const CiNamecmdb_ci_ucs_rack_unit = "cmdb_ci_ucs_rack_unit"
...
// This is essentially the same as CmdbCiUcsRackUnitGet but does not contain the "value", "link" and
...
//
type CmdbCiUcsRackUnitPost struct {
	Result struct {
		Attributes struct {
			...
			NumOfCores          string `json:"num_of_cores,omitempty"`
			...
			NumberOfAdapters    string `json:"number_of_adapters,omitempty"`
			...
			NumOfCpus           string `json:"num_of_cpus,omitempty"`
			...
			UcsEquipment        string `json:"ucs_equipment,omitempty"`
			...
			RackSerial          string `json:"rack_serial,omitempty"`
...
// The generateprovidersource utility constructs this STRUCT from metadata pulled from ServiceNow.
//
type CmdbCiUcsRackUnitGet struct {
	Result struct {
		Attributes struct {
			OsAddressWidth    string `json:"os_address_width,omitempty"`
			OperationalStatus string `json:"operational_status,omitempty"`
...
func ResourceCmdbCiUcsRackUnit() *schema.Resource {
	return &schema.Resource{
		Create: createResourceCmdbCiUcsRackUnit,
		Read:   readResourceCmdbCiUcsRackUnit,
		Update: updateResourceCmdbCiUcsRackUnit,
		Delete: deleteResourceCmdbCiUcsRackUnit,
...
//
func createResourceCmdbCiUcsRackUnit(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client) //Client Connection details
	// Use common function to update base attributes
	var ci CmdbCiUcsRackUnitPost
	if err := copyFromTerraformToServiceNowCmdbCiUcsRackUnit(resourceData, &ci); err != nil {
		return err
	}
	...
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_ucs_rack_unit + "?sysparm_fields=sys_id"
	...
	return readResourceCmdbCiUcsRackUnit(resourceData, serviceNowClient)
}

//  Read routine - This function is called when Terraform wants to create a new CI in the ServiceNow CMDB
//
func readResourceCmdbCiUcsRackUnit(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client)
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_ucs_rack_unit + "/" + resourceData.Id()
	var jsonData []byte
	...
	if err := copyFromServiceNowToTerraformCmdbCiUcsRackUnit(resourceData, jsonData); err != nil {
		return err
	}
...
//
func updateResourceCmdbCiUcsRackUnit(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	servicenowClient := serviceNowClient.(*Client)
...
//
func deleteResourceCmdbCiUcsRackUnit(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	...
	if err := resourceData.Set("install_status", "retired"); err != nil {
		return fmt.Errorf("CmdbCiUcsRackUnitfailed to set install_status field during destroy action %s", err)
	}
...
//
func copyFromTerraformToServiceNowCmdbCiUcsRackUnit(resourceData *schema.ResourceData, ci *CmdbCiUcsRackUnitPost) error {
	...
	attrs.State = resourceData.Get("state").(string)
	...
	attrs.NumOfCores = resourceData.Get("num_of_cores").(string)
	...
	attrs.Availability = resourceData.Get("availability").(string)
	...
	attrs.TotalMemory = resourceData.Get("total_memory").(string)
	attrs.Vendor = resourceData.Get("vendor.value").(string)
	attrs.ModelNumber = resourceData.Get("model_number").(string)
	attrs.NumberOfAdapters = resourceData.Get("number_of_adapters").(string)
	...
	attrs.Dn = resourceData.Get("dn").(string)
	attrs.Uuid = resourceData.Get("uuid").(string)
	...
	attrs.NumOfCpus = resourceData.Get("num_of_cpus").(string)
	attrs.ServerId = resourceData.Get("server_id").(string)
	...
	attrs.UcsEquipment = resourceData.Get("ucs_equipment.value").(string)
	...
	attrs.RackSerial = resourceData.Get("rack_serial").(string)
...
//
func copyFromServiceNowToTerraformCmdbCiUcsRackUnit(resourceData *schema.ResourceData, jsonData []byte) error {
	ci := CmdbCiUcsRackUnitGet{}
	...
	_ = resourceData.Set("state", attrs.State)
	...
	_ = resourceData.Set("num_of_cores", attrs.NumOfCores)
	...
	_ = resourceData.Set("availability", attrs.Availability)
	...
	_ = resourceData.Set("total_memory", attrs.TotalMemory)
	_ = resourceData.Set("vendor", StructToMap(attrs.Vendor))
	_ = resourceData.Set("model_number", attrs.ModelNumber)
	_ = resourceData.Set("number_of_adapters", attrs.NumberOfAdapters)
	...
	_ = resourceData.Set("dn", attrs.Dn)
	_ = resourceData.Set("uuid", attrs.Uuid)
	...
	_ = resourceData.Set("num_of_cpus", attrs.NumOfCpus)
	_ = resourceData.Set("server_id", attrs.ServerId)
	...
	_ = resourceData.Set("ucs_equipment", StructToMap(attrs.UcsEquipment))
	...
	_ = resourceData.Set("rack_serial", attrs.RackSerial)
