...
//
//  Description       :   This file is the resource provider for the cmdb_ci_appl_now_mid CMDB Class.  This code is executed
//                        when the servicenowcmdb_cmdb_ci_appl_now_mid keyword is used in a terraform script (*.tf) file
//
//                        This file is will need to be regenerated if the ServiceNow CMDB base CI Class
//                        "cmdb_ci" or of the cmdb_ci_appl_now_mid CI Class is modified.
///////////////////////////////////////////////////////////////////////////////////////////////////
import (
	"encoding/json"
	"fmt"
...
const CiNamecmdb_ci_appl_now_mid = "cmdb_ci_appl_now_mid"
...
// This is essentially the same as CmdbCiApplNowMidGet but does not contain the "value", "link" and
...
//
type CmdbCiApplNowMidPost struct {
	Result struct {
		Attributes struct {
			...
			MidServer                   string `json:"mid_server,omitempty"`
...
// The generateprovidersource utility constructs this STRUCT from metadata pulled from ServiceNow.
//
type CmdbCiApplNowMidGet struct {
	Result struct {
		Attributes struct {
			TcpPort                     string `json:"tcp_port,omitempty"`
			SkipSync                    string `json:"skip_sync,omitempty"`
...
func ResourceCmdbCiApplNowMid() *schema.Resource {
	return &schema.Resource{
		Create: createResourceCmdbCiApplNowMid,
		Read:   readResourceCmdbCiApplNowMid,
		Update: updateResourceCmdbCiApplNowMid,
		Delete: deleteResourceCmdbCiApplNowMid,
...
//
func createResourceCmdbCiApplNowMid(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client) //Client Connection details
	// Use common function to update base attributes
	var ci CmdbCiApplNowMidPost
	if err := copyFromTerraformToServiceNowCmdbCiApplNowMid(resourceData, &ci); err != nil {
		return err
	}
	...
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_appl_now_mid + "?sysparm_fields=sys_id"
	...
	return readResourceCmdbCiApplNowMid(resourceData, serviceNowClient)
}

//  Read routine - This function is called when Terraform wants to create a new CI in the ServiceNow CMDB
//
func readResourceCmdbCiApplNowMid(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client)
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_appl_now_mid + "/" + resourceData.Id()
	var jsonData []byte
	...
	if err := copyFromServiceNowToTerraformCmdbCiApplNowMid(resourceData, jsonData); err != nil {
		return err
	}
...
//
func updateResourceCmdbCiApplNowMid(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	servicenowClient := serviceNowClient.(*Client)
...
//
func deleteResourceCmdbCiApplNowMid(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	...
	if err := resourceData.Set("install_status", "retired"); err != nil {
		return fmt.Errorf("CmdbCiApplNowMidfailed to set install_status field during destroy action %s", err)
	}
...
//
func copyFromTerraformToServiceNowCmdbCiApplNowMid(resourceData *schema.ResourceData, ci *CmdbCiApplNowMidPost) error {
	...
	attrs.MidServer = resourceData.Get("mid_server.value").(string)
...
//
func copyFromServiceNowToTerraformCmdbCiApplNowMid(resourceData *schema.ResourceData, jsonData []byte) error {
	ci := CmdbCiApplNowMidGet{}
	...
	_ = resourceData.Set("mid_server", StructToMap(attrs.MidServer))
