...
//
//  Description       :   This file is the resource provider for the cmdb_ci_appl_doc_docbroker CMDB Class.  This code is executed
//                        when the servicenowcmdb_cmdb_ci_appl_doc_docbroker keyword is used in a terraform script (*.tf) file
//
//                        This file is will need to be regenerated if the ServiceNow CMDB base CI Class
//                        "cmdb_ci" or of the cmdb_ci_appl_doc_docbroker CI Class is modified.
///////////////////////////////////////////////////////////////////////////////////////////////////
import (
	"encoding/json"
	"fmt"
...
const CiNamecmdb_ci_appl_doc_docbroker = "cmdb_ci_appl_doc_docbroker"
...
// This is essentially the same as CmdbCiApplDocDocbrokerGet but does not contain the "value", "link" and
...
//
type CmdbCiApplDocDocbrokerPost struct {
	Result struct {
		Attributes struct {
...
// The generateprovidersource utility constructs this STRUCT from metadata pulled from ServiceNow.
//
type CmdbCiApplDocDocbrokerGet struct {
	Result struct {
		Attributes struct {
			TcpPort                     string `json:"tcp_port,omitempty"`
			SkipSync                    string `json:"skip_sync,omitempty"`
...
func ResourceCmdbCiApplDocDocbroker() *schema.Resource {
	return &schema.Resource{
		Create: createResourceCmdbCiApplDocDocbroker,
		Read:   readResourceCmdbCiApplDocDocbroker,
		Update: updateResourceCmdbCiApplDocDocbroker,
		Delete: deleteResourceCmdbCiApplDocDocbroker,
...
//
func createResourceCmdbCiApplDocDocbroker(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client) //Client Connection details
	// Use common function to update base attributes
	var ci CmdbCiApplDocDocbrokerPost
	if err := copyFromTerraformToServiceNowCmdbCiApplDocDocbroker(resourceData, &ci); err != nil {
		return err
	}
	...
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_appl_doc_docbroker + "?sysparm_fields=sys_id"
	...
	return readResourceCmdbCiApplDocDocbroker(resourceData, serviceNowClient)
}

//  Read routine - This function is called when Terraform wants to create a new CI in the ServiceNow CMDB
//
func readResourceCmdbCiApplDocDocbroker(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client)
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_appl_doc_docbroker + "/" + resourceData.Id()
	var jsonData []byte
	...
	if err := copyFromServiceNowToTerraformCmdbCiApplDocDocbroker(resourceData, jsonData); err != nil {
		return err
	}
...
//
func updateResourceCmdbCiApplDocDocbroker(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	servicenowClient := serviceNowClient.(*Client)
...
//
func deleteResourceCmdbCiApplDocDocbroker(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	...
	if err := resourceData.Set("install_status", "retired"); err != nil {
		return fmt.Errorf("CmdbCiApplDocDocbrokerfailed to set install_status field during destroy action %s", err)
	}
...
//
func copyFromTerraformToServiceNowCmdbCiApplDocDocbroker(resourceData *schema.ResourceData, ci *CmdbCiApplDocDocbrokerPost) error {
...
//
func copyFromServiceNowToTerraformCmdbCiApplDocDocbroker(resourceData *schema.ResourceData, jsonData []byte) error {
	ci := CmdbCiApplDocDocbrokerGet{}
