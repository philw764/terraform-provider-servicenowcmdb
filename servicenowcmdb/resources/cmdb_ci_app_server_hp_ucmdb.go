...
//
//  Description       :   This file is the resource provider for the cmdb_ci_app_server_hp_ucmdb CMDB Class.  This code is executed
//                        when the servicenowcmdb_cmdb_ci_app_server_hp_ucmdb keyword is used in a terraform script (*.tf) file
//
//                        This file is will need to be regenerated if the ServiceNow CMDB base CI Class
//                        "cmdb_ci" or of the cmdb_ci_app_server_hp_ucmdb CI Class is modified.
///////////////////////////////////////////////////////////////////////////////////////////////////
import (
	"encoding/json"
	"fmt"
...
const CiNamecmdb_ci_app_server_hp_ucmdb = "cmdb_ci_app_server_hp_ucmdb"
...
// This is essentially the same as CmdbCiAppServerHpUcmdbGet but does not contain the "value", "link" and
...
//
type CmdbCiAppServerHpUcmdbPost struct {
	Result struct {
		Attributes struct {
...
// The generateprovidersource utility constructs this STRUCT from metadata pulled from ServiceNow.
//
type CmdbCiAppServerHpUcmdbGet struct {
	Result struct {
		Attributes struct {
			Container                   string `json:"container,omitempty"`
			TcpPort                     string `json:"tcp_port,omitempty"`
...
func ResourceCmdbCiAppServerHpUcmdb() *schema.Resource {
	return &schema.Resource{
		Create: createResourceCmdbCiAppServerHpUcmdb,
		Read:   readResourceCmdbCiAppServerHpUcmdb,
		Update: updateResourceCmdbCiAppServerHpUcmdb,
		Delete: deleteResourceCmdbCiAppServerHpUcmdb,
...
//
func createResourceCmdbCiAppServerHpUcmdb(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client) //Client Connection details
	// Use common function to update base attributes
	var ci CmdbCiAppServerHpUcmdbPost
	if err := copyFromTerraformToServiceNowCmdbCiAppServerHpUcmdb(resourceData, &ci); err != nil {
		return err
	}
	...
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_app_server_hp_ucmdb + "?sysparm_fields=sys_id"
	...
	return readResourceCmdbCiAppServerHpUcmdb(resourceData, serviceNowClient)
}

//  Read routine - This function is called when Terraform wants to create a new CI in the ServiceNow CMDB
//
func readResourceCmdbCiAppServerHpUcmdb(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client)
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_app_server_hp_ucmdb + "/" + resourceData.Id()
	var jsonData []byte
	...
	if err := copyFromServiceNowToTerraformCmdbCiAppServerHpUcmdb(resourceData, jsonData); err != nil {
		return err
	}
...
//
func updateResourceCmdbCiAppServerHpUcmdb(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	servicenowClient := serviceNowClient.(*Client)
...
//
func deleteResourceCmdbCiAppServerHpUcmdb(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	...
	if err := resourceData.Set("install_status", "retired"); err != nil {
		return fmt.Errorf("CmdbCiAppServerHpUcmdbfailed to set install_status field during destroy action %s", err)
	}
...
//
func copyFromTerraformToServiceNowCmdbCiAppServerHpUcmdb(resourceData *schema.ResourceData, ci *CmdbCiAppServerHpUcmdbPost) error {
...
//
func copyFromServiceNowToTerraformCmdbCiAppServerHpUcmdb(resourceData *schema.ResourceData, jsonData []byte) error {
	ci := CmdbCiAppServerHpUcmdbGet{}
