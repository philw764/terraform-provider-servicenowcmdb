...
//
//  Description       :   This file is the resource provider for the cmdb_ci_san_zone CMDB Class.  This code is executed
//                        when the servicenowcmdb_cmdb_ci_san_zone keyword is used in a terraform script (*.tf) file
//
//                        This file is will need to be regenerated if the ServiceNow CMDB base CI Class
//                        "cmdb_ci" or of the cmdb_ci_san_zone CI Class is modified.
///////////////////////////////////////////////////////////////////////////////////////////////////
import (
...
const CiNamecmdb_ci_san_zone  = "cmdb_ci_san_zone"
...
// This is essentially the same as CmdbCiSanZoneGet but does not contain the "value", "link" and
...
Attributes   struct {
            ...
            ZoneSet    string `json:"zone_set,omitempty"`
...
func ResourceCmdbCiSanZone() *schema.Resource {
return &schema.Resource{
Create: createResourceCmdbCiSanZone,
Read:   readResourceCmdbCiSanZone,
Update: updateResourceCmdbCiSanZone,
Delete: deleteResourceCmdbCiSanZone,
...
//
func createResourceCmdbCiSanZone(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

servicenowClient := serviceNowClient.(*Client) //Client Connection details
// Use common function to update base attributes
var ci CmdbCiSanZonePost
if err := copyFromTerraformToServiceNowCmdbCiSanZone(resourceData, &ci); err != nil {
return err
}
...
SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_san_zone + "?sysparm_fields=sys_id"
...
return readResourceCmdbCiSanZone(resourceData, serviceNowClient)
}

//  Read routine - This function is called when Terraform wants to create a new CI in the ServiceNow CMDB
//
func readResourceCmdbCiSanZone(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

servicenowClient := serviceNowClient.(*Client)
SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_san_zone + "/" + resourceData.Id()
var jsonData []byte
...
if err := copyFromServiceNowToTerraformCmdbCiSanZone(resourceData,jsonData);  err != nil {
return err
}
...
//
func updateResourceCmdbCiSanZone(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
servicenowClient := serviceNowClient.(*Client)
...
//
func deleteResourceCmdbCiSanZone(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
...
return fmt.Errorf("CmdbCiSanZonefailed to set install_status field during destroy action %s", err)
}
...
//
func copyFromTerraformToServiceNowCmdbCiSanZone (resourceData *schema.ResourceData, ci *CmdbCiSanZonePost) error {
...
attrs := &ci.Result.Attributes
                ...
                attrs.ZoneId  = resourceData.Get("zone_id").(string)
                ...
                attrs.ZoneSet  = resourceData.Get("zone_set.value").(string)
...
//
func copyFromServiceNowToTerraformCmdbCiSanZone (resourceData *schema.ResourceData, jsonData [] byte) error {
ci := CmdbCiSanZoneGet{}
...
attrs := ci.Result.Attributes
                ...
                _ = resourceData.Set("zone_id", attrs.ZoneId)
                ...
                _ = resourceData.Set("zone_set", StructToMap(attrs.ZoneSet))
