...
//
//  Description       :   This file is the resource provider for the cmdb_ci_appl_cisco_call_man CMDB Class.  This code is executed
//                        when the servicenowcmdb_cmdb_ci_appl_cisco_call_man keyword is used in a terraform script (*.tf) file
//
//                        This file is will need to be regenerated if the ServiceNow CMDB base CI Class
//                        "cmdb_ci" or of the cmdb_ci_appl_cisco_call_man CI Class is modified.
///////////////////////////////////////////////////////////////////////////////////////////////////
import (
	"encoding/json"
	"fmt"
...
const CiNamecmdb_ci_appl_cisco_call_man = "cmdb_ci_appl_cisco_call_man"
...
// This is essentially the same as CmdbCiApplCiscoCallManGet but does not contain the "value", "link" and
...
//
type CmdbCiApplCiscoCallManPost struct {
	Result struct {
		Attributes struct {
...
// The generateprovidersource utility constructs this STRUCT from metadata pulled from ServiceNow.
//
type CmdbCiApplCiscoCallManGet struct {
	Result struct {
		Attributes struct {
			TcpPort                     string `json:"tcp_port,omitempty"`
			SkipSync                    string `json:"skip_sync,omitempty"`
...
func ResourceCmdbCiApplCiscoCallMan() *schema.Resource {
	return &schema.Resource{
		Create: createResourceCmdbCiApplCiscoCallMan,
		Read:   readResourceCmdbCiApplCiscoCallMan,
		Update: updateResourceCmdbCiApplCiscoCallMan,
		Delete: deleteResourceCmdbCiApplCiscoCallMan,
...
//
func createResourceCmdbCiApplCiscoCallMan(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client) //Client Connection details
	// Use common function to update base attributes
	var ci CmdbCiApplCiscoCallManPost
	if err := copyFromTerraformToServiceNowCmdbCiApplCiscoCallMan(resourceData, &ci); err != nil {
		return err
	}
	...
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_appl_cisco_call_man + "?sysparm_fields=sys_id"
	...
	return readResourceCmdbCiApplCiscoCallMan(resourceData, serviceNowClient)
}

//  Read routine - This function is called when Terraform wants to create a new CI in the ServiceNow CMDB
//
func readResourceCmdbCiApplCiscoCallMan(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client)
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_appl_cisco_call_man + "/" + resourceData.Id()
	var jsonData []byte
	...
	if err := copyFromServiceNowToTerraformCmdbCiApplCiscoCallMan(resourceData, jsonData); err != nil {
		return err
	}
...
//
func updateResourceCmdbCiApplCiscoCallMan(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	servicenowClient := serviceNowClient.(*Client)
...
//
func deleteResourceCmdbCiApplCiscoCallMan(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	...
	if err := resourceData.Set("install_status", "retired"); err != nil {
		return fmt.Errorf("CmdbCiApplCiscoCallManfailed to set install_status field during destroy action %s", err)
	}
...
//
func copyFromTerraformToServiceNowCmdbCiApplCiscoCallMan(resourceData *schema.ResourceData, ci *CmdbCiApplCiscoCallManPost) error {
...
//
func copyFromServiceNowToTerraformCmdbCiApplCiscoCallMan(resourceData *schema.ResourceData, jsonData []byte) error {
	ci := CmdbCiApplCiscoCallManGet{}
