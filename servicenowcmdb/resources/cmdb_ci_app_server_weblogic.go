...
//
//  Description       :   This file is the resource provider for the cmdb_ci_app_server_weblogic CMDB Class.  This code is executed
//                        when the servicenowcmdb_cmdb_ci_app_server_weblogic keyword is used in a terraform script (*.tf) file
//
//                        This file is will need to be regenerated if the ServiceNow CMDB base CI Class
//                        "cmdb_ci" or of the cmdb_ci_app_server_weblogic CI Class is modified.
///////////////////////////////////////////////////////////////////////////////////////////////////
import (
	"encoding/json"
	"fmt"
...
const CiNamecmdb_ci_app_server_weblogic = "cmdb_ci_app_server_weblogic"
...
// This is essentially the same as CmdbCiAppServerWeblogicGet but does not contain the "value", "link" and
...
//
type CmdbCiAppServerWeblogicPost struct {
	Result struct {
		Attributes struct {
			...
			DomainDirectory             string `json:"domain_directory,omitempty"`
			...
			WlsHome                     string `json:"wls_home,omitempty"`
			...
			WeblogicDomain              string `json:"weblogic_domain,omitempty"`
...
// The generateprovidersource utility constructs this STRUCT from metadata pulled from ServiceNow.
//
type CmdbCiAppServerWeblogicGet struct {
	Result struct {
		Attributes struct {
			Container                   string `json:"container,omitempty"`
			TcpPort                     string `json:"tcp_port,omitempty"`
...
func ResourceCmdbCiAppServerWeblogic() *schema.Resource {
	return &schema.Resource{
		Create: createResourceCmdbCiAppServerWeblogic,
		Read:   readResourceCmdbCiAppServerWeblogic,
		Update: updateResourceCmdbCiAppServerWeblogic,
		Delete: deleteResourceCmdbCiAppServerWeblogic,
...
//
func createResourceCmdbCiAppServerWeblogic(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client) //Client Connection details
	// Use common function to update base attributes
	var ci CmdbCiAppServerWeblogicPost
	if err := copyFromTerraformToServiceNowCmdbCiAppServerWeblogic(resourceData, &ci); err != nil {
		return err
	}
	...
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_app_server_weblogic + "?sysparm_fields=sys_id"
	...
	return readResourceCmdbCiAppServerWeblogic(resourceData, serviceNowClient)
}

//  Read routine - This function is called when Terraform wants to create a new CI in the ServiceNow CMDB
//
func readResourceCmdbCiAppServerWeblogic(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client)
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_app_server_weblogic + "/" + resourceData.Id()
	var jsonData []byte
	...
	if err := copyFromServiceNowToTerraformCmdbCiAppServerWeblogic(resourceData, jsonData); err != nil {
		return err
	}
...
//
func updateResourceCmdbCiAppServerWeblogic(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	servicenowClient := serviceNowClient.(*Client)
...
//
func deleteResourceCmdbCiAppServerWeblogic(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	...
	if err := resourceData.Set("install_status", "retired"); err != nil {
		return fmt.Errorf("CmdbCiAppServerWeblogicfailed to set install_status field during destroy action %s", err)
	}
...
//
func copyFromTerraformToServiceNowCmdbCiAppServerWeblogic(resourceData *schema.ResourceData, ci *CmdbCiAppServerWeblogicPost) error {
	...
	attrs.DomainDirectory = resourceData.Get("domain_directory").(string)
	...
	attrs.WlsHome = resourceData.Get("wls_home").(string)
	...
	attrs.ServerName = resourceData.Get("server_name").(string)
	attrs.ShortDescription = resourceData.Get("short_description").(string)
	attrs.WeblogicDomain = resourceData.Get("weblogic_domain").(string)
...
//
func copyFromServiceNowToTerraformCmdbCiAppServerWeblogic(resourceData *schema.ResourceData, jsonData []byte) error {
	ci := CmdbCiAppServerWeblogicGet{}
	...
	_ = resourceData.Set("domain_directory", attrs.DomainDirectory)
	...
	_ = resourceData.Set("wls_home", attrs.WlsHome)
	...
	_ = resourceData.Set("server_name", attrs.ServerName)
	_ = resourceData.Set("short_description", attrs.ShortDescription)
	_ = resourceData.Set("weblogic_domain", attrs.WeblogicDomain)
