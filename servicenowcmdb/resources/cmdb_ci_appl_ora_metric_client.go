...
//
//  Description       :   This file is the resource provider for the cmdb_ci_appl_ora_metric_client CMDB Class.  This code is executed
//                        when the servicenowcmdb_cmdb_ci_appl_ora_metric_client keyword is used in a terraform script (*.tf) file
//
//                        This file is will need to be regenerated if the ServiceNow CMDB base CI Class
//                        "cmdb_ci" or of the cmdb_ci_appl_ora_metric_client CI Class is modified.
///////////////////////////////////////////////////////////////////////////////////////////////////
import (
	"encoding/json"
	"fmt"
...
const CiNamecmdb_ci_appl_ora_metric_client = "cmdb_ci_appl_ora_metric_client"
...
// This is essentially the same as CmdbCiApplOraMetricClientGet but does not contain the "value", "link" and
...
//
type CmdbCiApplOraMetricClientPost struct {
	Result struct {
		Attributes struct {
...
// The generateprovidersource utility constructs this STRUCT from metadata pulled from ServiceNow.
//
type CmdbCiApplOraMetricClientGet struct {
	Result struct {
		Attributes struct {
			TcpPort                     string `json:"tcp_port,omitempty"`
			SkipSync                    string `json:"skip_sync,omitempty"`
...
func ResourceCmdbCiApplOraMetricClient() *schema.Resource {
	return &schema.Resource{
		Create: createResourceCmdbCiApplOraMetricClient,
		Read:   readResourceCmdbCiApplOraMetricClient,
		Update: updateResourceCmdbCiApplOraMetricClient,
		Delete: deleteResourceCmdbCiApplOraMetricClient,
...
//
func createResourceCmdbCiApplOraMetricClient(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client) //Client Connection details
	// Use common function to update base attributes
	var ci CmdbCiApplOraMetricClientPost
	if err := copyFromTerraformToServiceNowCmdbCiApplOraMetricClient(resourceData, &ci); err != nil {
		return err
	}
	...
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_appl_ora_metric_client + "?sysparm_fields=sys_id"
	...
	return readResourceCmdbCiApplOraMetricClient(resourceData, serviceNowClient)
}

//  Read routine - This function is called when Terraform wants to create a new CI in the ServiceNow CMDB
//
func readResourceCmdbCiApplOraMetricClient(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client)
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_appl_ora_metric_client + "/" + resourceData.Id()
	var jsonData []byte
	...
	if err := copyFromServiceNowToTerraformCmdbCiApplOraMetricClient(resourceData, jsonData); err != nil {
		return err
	}
...
//
func updateResourceCmdbCiApplOraMetricClient(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	servicenowClient := serviceNowClient.(*Client)
...
//
func deleteResourceCmdbCiApplOraMetricClient(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	...
	if err := resourceData.Set("install_status", "retired"); err != nil {
		return fmt.Errorf("CmdbCiApplOraMetricClientfailed to set install_status field during destroy action %s", err)
	}
...
//
func copyFromTerraformToServiceNowCmdbCiApplOraMetricClient(resourceData *schema.ResourceData, ci *CmdbCiApplOraMetricClientPost) error {
...
//
func copyFromServiceNowToTerraformCmdbCiApplOraMetricClient(resourceData *schema.ResourceData, jsonData []byte) error {
	ci := CmdbCiApplOraMetricClientGet{}
