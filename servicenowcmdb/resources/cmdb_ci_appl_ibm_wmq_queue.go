...
//
//  Description       :   This file is the resource provider for the cmdb_ci_appl_ibm_wmq_queue CMDB Class.  This code is executed
//                        when the servicenowcmdb_cmdb_ci_appl_ibm_wmq_queue keyword is used in a terraform script (*.tf) file
//
//                        This file is will need to be regenerated if the ServiceNow CMDB base CI Class
//                        "cmdb_ci" or of the cmdb_ci_appl_ibm_wmq_queue CI Class is modified.
///////////////////////////////////////////////////////////////////////////////////////////////////
import (
	"encoding/json"
	"fmt"
...
const CiNamecmdb_ci_appl_ibm_wmq_queue = "cmdb_ci_appl_ibm_wmq_queue"
...
// This is essentially the same as CmdbCiApplIbmWmqQueueGet but does not contain the "value", "link" and
...
//
type CmdbCiApplIbmWmqQueuePost struct {
	Result struct {
		Attributes struct {
...
// The generateprovidersource utility constructs this STRUCT from metadata pulled from ServiceNow.
//
type CmdbCiApplIbmWmqQueueGet struct {
	Result struct {
		Attributes struct {
			TcpPort                     string `json:"tcp_port,omitempty"`
			SkipSync                    string `json:"skip_sync,omitempty"`
...
func ResourceCmdbCiApplIbmWmqQueue() *schema.Resource {
	return &schema.Resource{
		Create: createResourceCmdbCiApplIbmWmqQueue,
		Read:   readResourceCmdbCiApplIbmWmqQueue,
		Update: updateResourceCmdbCiApplIbmWmqQueue,
		Delete: deleteResourceCmdbCiApplIbmWmqQueue,
...
//
func createResourceCmdbCiApplIbmWmqQueue(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client) //Client Connection details
	// Use common function to update base attributes
	var ci CmdbCiApplIbmWmqQueuePost
	if err := copyFromTerraformToServiceNowCmdbCiApplIbmWmqQueue(resourceData, &ci); err != nil {
		return err
	}
	...
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_appl_ibm_wmq_queue + "?sysparm_fields=sys_id"
	...
	return readResourceCmdbCiApplIbmWmqQueue(resourceData, serviceNowClient)
}

//  Read routine - This function is called when Terraform wants to create a new CI in the ServiceNow CMDB
//
func readResourceCmdbCiApplIbmWmqQueue(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client)
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_appl_ibm_wmq_queue + "/" + resourceData.Id()
	var jsonData []byte
	...
	if err := copyFromServiceNowToTerraformCmdbCiApplIbmWmqQueue(resourceData, jsonData); err != nil {
		return err
	}
...
//
func updateResourceCmdbCiApplIbmWmqQueue(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	servicenowClient := serviceNowClient.(*Client)
...
//
func deleteResourceCmdbCiApplIbmWmqQueue(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	...
	if err := resourceData.Set("install_status", "retired"); err != nil {
		return fmt.Errorf("CmdbCiApplIbmWmqQueuefailed to set install_status field during destroy action %s", err)
	}
...
//
func copyFromTerraformToServiceNowCmdbCiApplIbmWmqQueue(resourceData *schema.ResourceData, ci *CmdbCiApplIbmWmqQueuePost) error {
...
//
func copyFromServiceNowToTerraformCmdbCiApplIbmWmqQueue(resourceData *schema.ResourceData, jsonData []byte) error {
	ci := CmdbCiApplIbmWmqQueueGet{}
