...
//
//  Description       :   This file is the resource provider for the cmdb_ci_appl_ibm_ctg CMDB Class.  This code is executed
//                        when the servicenowcmdb_cmdb_ci_appl_ibm_ctg keyword is used in a terraform script (*.tf) file
//
//                        This file is will need to be regenerated if the ServiceNow CMDB base CI Class
//                        "cmdb_ci" or of the cmdb_ci_appl_ibm_ctg CI Class is modified.
///////////////////////////////////////////////////////////////////////////////////////////////////
import (
	"encoding/json"
	"fmt"
...
const CiNamecmdb_ci_appl_ibm_ctg = "cmdb_ci_appl_ibm_ctg"
...
// This is essentially the same as CmdbCiApplIbmCtgGet but does not contain the "value", "link" and
...
//
type CmdbCiApplIbmCtgPost struct {
	Result struct {
		Attributes struct {
...
// The generateprovidersource utility constructs this STRUCT from metadata pulled from ServiceNow.
//
type CmdbCiApplIbmCtgGet struct {
	Result struct {
		Attributes struct {
			TcpPort                     string `json:"tcp_port,omitempty"`
			SkipSync                    string `json:"skip_sync,omitempty"`
...
func ResourceCmdbCiApplIbmCtg() *schema.Resource {
	return &schema.Resource{
		Create: createResourceCmdbCiApplIbmCtg,
		Read:   readResourceCmdbCiApplIbmCtg,
		Update: updateResourceCmdbCiApplIbmCtg,
		Delete: deleteResourceCmdbCiApplIbmCtg,
...
//
func createResourceCmdbCiApplIbmCtg(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client) //Client Connection details
	// Use common function to update base attributes
	var ci CmdbCiApplIbmCtgPost
	if err := copyFromTerraformToServiceNowCmdbCiApplIbmCtg(resourceData, &ci); err != nil {
		return err
	}
	...
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_appl_ibm_ctg + "?sysparm_fields=sys_id"
	...
	return readResourceCmdbCiApplIbmCtg(resourceData, serviceNowClient)
}

//  Read routine - This function is called when Terraform wants to create a new CI in the ServiceNow CMDB
//
func readResourceCmdbCiApplIbmCtg(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client)
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_appl_ibm_ctg + "/" + resourceData.Id()
	var jsonData []byte
	...
	if err := copyFromServiceNowToTerraformCmdbCiApplIbmCtg(resourceData, jsonData); err != nil {
		return err
	}
...
//
func updateResourceCmdbCiApplIbmCtg(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	servicenowClient := serviceNowClient.(*Client)
...
//
func deleteResourceCmdbCiApplIbmCtg(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	...
	if err := resourceData.Set("install_status", "retired"); err != nil {
		return fmt.Errorf("CmdbCiApplIbmCtgfailed to set install_status field during destroy action %s", err)
	}
...
//
func copyFromTerraformToServiceNowCmdbCiApplIbmCtg(resourceData *schema.ResourceData, ci *CmdbCiApplIbmCtgPost) error {
...
//
func copyFromServiceNowToTerraformCmdbCiApplIbmCtg(resourceData *schema.ResourceData, jsonData []byte) error {
	ci := CmdbCiApplIbmCtgGet{}
