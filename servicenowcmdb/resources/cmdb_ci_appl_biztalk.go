...
//
//  Description       :   This file is the resource provider for the cmdb_ci_appl_biztalk CMDB Class.  This code is executed
//                        when the servicenowcmdb_cmdb_ci_appl_biztalk keyword is used in a terraform script (*.tf) file
//
//                        This file is will need to be regenerated if the ServiceNow CMDB base CI Class
//                        "cmdb_ci" or of the cmdb_ci_appl_biztalk CI Class is modified.
///////////////////////////////////////////////////////////////////////////////////////////////////
import (
	"encoding/json"
	"fmt"
...
const CiNamecmdb_ci_appl_biztalk = "cmdb_ci_appl_biztalk"
...
// This is essentially the same as CmdbCiApplBiztalkGet but does not contain the "value", "link" and
...
//
type CmdbCiApplBiztalkPost struct {
	Result struct {
		Attributes struct {
...
// The generateprovidersource utility constructs this STRUCT from metadata pulled from ServiceNow.
//
type CmdbCiApplBiztalkGet struct {
	Result struct {
		Attributes struct {
			TcpPort                     string `json:"tcp_port,omitempty"`
			SkipSync                    string `json:"skip_sync,omitempty"`
...
func ResourceCmdbCiApplBiztalk() *schema.Resource {
	return &schema.Resource{
		Create: createResourceCmdbCiApplBiztalk,
		Read:   readResourceCmdbCiApplBiztalk,
		Update: updateResourceCmdbCiApplBiztalk,
		Delete: deleteResourceCmdbCiApplBiztalk,
...
//
func createResourceCmdbCiApplBiztalk(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client) //Client Connection details
	// Use common function to update base attributes
	var ci CmdbCiApplBiztalkPost
	if err := copyFromTerraformToServiceNowCmdbCiApplBiztalk(resourceData, &ci); err != nil {
		return err
	}
	...
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_appl_biztalk + "?sysparm_fields=sys_id"
	...
	return readResourceCmdbCiApplBiztalk(resourceData, serviceNowClient)
}

//  Read routine - This function is called when Terraform wants to create a new CI in the ServiceNow CMDB
//
func readResourceCmdbCiApplBiztalk(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client)
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_appl_biztalk + "/" + resourceData.Id()
	var jsonData []byte
	...
	if err := copyFromServiceNowToTerraformCmdbCiApplBiztalk(resourceData, jsonData); err != nil {
		return err
	}
...
//
func updateResourceCmdbCiApplBiztalk(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	servicenowClient := serviceNowClient.(*Client)
...
//
func deleteResourceCmdbCiApplBiztalk(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	...
	if err := resourceData.Set("install_status", "retired"); err != nil {
		return fmt.Errorf("CmdbCiApplBiztalkfailed to set install_status field during destroy action %s", err)
	}
...
//
func copyFromTerraformToServiceNowCmdbCiApplBiztalk(resourceData *schema.ResourceData, ci *CmdbCiApplBiztalkPost) error {
...
//
func copyFromServiceNowToTerraformCmdbCiApplBiztalk(resourceData *schema.ResourceData, jsonData []byte) error {
	ci := CmdbCiApplBiztalkGet{}
