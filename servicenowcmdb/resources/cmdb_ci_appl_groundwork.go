...
//
//  Description       :   This file is the resource provider for the cmdb_ci_appl_groundwork CMDB Class.  This code is executed
//                        when the servicenowcmdb_cmdb_ci_appl_groundwork keyword is used in a terraform script (*.tf) file
//
//                        This file is will need to be regenerated if the ServiceNow CMDB base CI Class
//                        "cmdb_ci" or of the cmdb_ci_appl_groundwork CI Class is modified.
///////////////////////////////////////////////////////////////////////////////////////////////////
import (
	"encoding/json"
	"fmt"
...
const CiNamecmdb_ci_appl_groundwork = "cmdb_ci_appl_groundwork"
...
// This is essentially the same as CmdbCiApplGroundworkGet but does not contain the "value", "link" and
...
//
type CmdbCiApplGroundworkPost struct {
	Result struct {
		Attributes struct {
...
// The generateprovidersource utility constructs this STRUCT from metadata pulled from ServiceNow.
//
type CmdbCiApplGroundworkGet struct {
	Result struct {
		Attributes struct {
			TcpPort                     string `json:"tcp_port,omitempty"`
			SkipSync                    string `json:"skip_sync,omitempty"`
...
func ResourceCmdbCiApplGroundwork() *schema.Resource {
	return &schema.Resource{
		Create: createResourceCmdbCiApplGroundwork,
		Read:   readResourceCmdbCiApplGroundwork,
		Update: updateResourceCmdbCiApplGroundwork,
		Delete: deleteResourceCmdbCiApplGroundwork,
...
//
func createResourceCmdbCiApplGroundwork(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client) //Client Connection details
	// Use common function to update base attributes
	var ci CmdbCiApplGroundworkPost
	if err := copyFromTerraformToServiceNowCmdbCiApplGroundwork(resourceData, &ci); err != nil {
		return err
	}
	...
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_appl_groundwork + "?sysparm_fields=sys_id"
	...
	return readResourceCmdbCiApplGroundwork(resourceData, serviceNowClient)
}

//  Read routine - This function is called when Terraform wants to create a new CI in the ServiceNow CMDB
//
func readResourceCmdbCiApplGroundwork(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client)
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_appl_groundwork + "/" + resourceData.Id()
	var jsonData []byte
	...
	if err := copyFromServiceNowToTerraformCmdbCiApplGroundwork(resourceData, jsonData); err != nil {
		return err
	}
...
//
func updateResourceCmdbCiApplGroundwork(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	servicenowClient := serviceNowClient.(*Client)
...
//
func deleteResourceCmdbCiApplGroundwork(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	...
	if err := resourceData.Set("install_status", "retired"); err != nil {
		return fmt.Errorf("CmdbCiApplGroundworkfailed to set install_status field during destroy action %s", err)
	}
...
//
func copyFromTerraformToServiceNowCmdbCiApplGroundwork(resourceData *schema.ResourceData, ci *CmdbCiApplGroundworkPost) error {
...
//
func copyFromServiceNowToTerraformCmdbCiApplGroundwork(resourceData *schema.ResourceData, jsonData []byte) error {
	ci := CmdbCiApplGroundworkGet{}
