...
//
//  Description       :   This file is the resource provider for the cmdb_ci_iscsi_export CMDB Class.  This code is executed
//                        when the servicenowcmdb_cmdb_ci_iscsi_export keyword is used in a terraform script (*.tf) file
//
//                        This file is will need to be regenerated if the ServiceNow CMDB base CI Class
//                        "cmdb_ci" or of the cmdb_ci_iscsi_export CI Class is modified.
///////////////////////////////////////////////////////////////////////////////////////////////////
import (
	"encoding/json"
	"fmt"
...
const CiNamecmdb_ci_iscsi_export = "cmdb_ci_iscsi_export"
...
// This is essentially the same as CmdbCiIscsiExportGet but does not contain the "value", "link" and
...
//
type CmdbCiIscsiExportPost struct {
	Result struct {
		Attributes struct {
...
// The generateprovidersource utility constructs this STRUCT from metadata pulled from ServiceNow.
//
type CmdbCiIscsiExportGet struct {
	Result struct {
		Attributes struct {
			SkipSync           string `json:"skip_sync,omitempty"`
			OperationalStatus  string `json:"operational_status,omitempty"`
...
func ResourceCmdbCiIscsiExport() *schema.Resource {
	return &schema.Resource{
		Create: createResourceCmdbCiIscsiExport,
		Read:   readResourceCmdbCiIscsiExport,
		Update: updateResourceCmdbCiIscsiExport,
		Delete: deleteResourceCmdbCiIscsiExport,
...
//
func createResourceCmdbCiIscsiExport(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client) //Client Connection details
	// Use common function to update base attributes
	var ci CmdbCiIscsiExportPost
	if err := copyFromTerraformToServiceNowCmdbCiIscsiExport(resourceData, &ci); err != nil {
		return err
	}
	...
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_iscsi_export + "?sysparm_fields=sys_id"
	...
	return readResourceCmdbCiIscsiExport(resourceData, serviceNowClient)
}

//  Read routine - This function is called when Terraform wants to create a new CI in the ServiceNow CMDB
//
func readResourceCmdbCiIscsiExport(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client)
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_iscsi_export + "/" + resourceData.Id()
	var jsonData []byte
	...
	if err := copyFromServiceNowToTerraformCmdbCiIscsiExport(resourceData, jsonData); err != nil {
		return err
	}
...
//
func updateResourceCmdbCiIscsiExport(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	servicenowClient := serviceNowClient.(*Client)
...
//
func deleteResourceCmdbCiIscsiExport(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	...
	if err := resourceData.Set("install_status", "retired"); err != nil {
		return fmt.Errorf("CmdbCiIscsiExportfailed to set install_status field during destroy action %s", err)
	}
...
//
func copyFromTerraformToServiceNowCmdbCiIscsiExport(resourceData *schema.ResourceData, ci *CmdbCiIscsiExportPost) error {
	...
	attrs.Lun = resourceData.Get("lun").(string)
	attrs.SysUpdatedOn = resourceData.Get("sys_updated_on").(string)
	attrs.InitiatorIqn = resourceData.Get("initiator_iqn").(string)
	...
	attrs.ExportId = resourceData.Get("export_id").(string)
	...
	attrs.ExportedBy = resourceData.Get("exported_by.value").(string)
	...
	attrs.HostedBy = resourceData.Get("hosted_by.value").(string)
	...
	attrs.Iqn = resourceData.Get("iqn").(string)
	attrs.Storage = resourceData.Get("storage.value").(string)
...
//
func copyFromServiceNowToTerraformCmdbCiIscsiExport(resourceData *schema.ResourceData, jsonData []byte) error {
	ci := CmdbCiIscsiExportGet{}
	...
	_ = resourceData.Set("lun", attrs.Lun)
	_ = resourceData.Set("sys_updated_on", attrs.SysUpdatedOn)
	_ = resourceData.Set("initiator_iqn", attrs.InitiatorIqn)
	...
	_ = resourceData.Set("export_id", attrs.ExportId)
	...
	_ = resourceData.Set("exported_by", StructToMap(attrs.ExportedBy))
	...
	_ = resourceData.Set("hosted_by", StructToMap(attrs.HostedBy))
	...
	_ = resourceData.Set("iqn", attrs.Iqn)
	_ = resourceData.Set("storage", StructToMap(attrs.Storage))
