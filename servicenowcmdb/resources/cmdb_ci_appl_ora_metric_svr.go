...
//
//  Description       :   This file is the resource provider for the cmdb_ci_appl_ora_metric_svr CMDB Class.  This code is executed
//                        when the servicenowcmdb_cmdb_ci_appl_ora_metric_svr keyword is used in a terraform script (*.tf) file
//
//                        This file is will need to be regenerated if the ServiceNow CMDB base CI Class
//                        "cmdb_ci" or of the cmdb_ci_appl_ora_metric_svr CI Class is modified.
///////////////////////////////////////////////////////////////////////////////////////////////////
import (
	"encoding/json"
	"fmt"
...
const CiNamecmdb_ci_appl_ora_metric_svr = "cmdb_ci_appl_ora_metric_svr"
...
// This is essentially the same as CmdbCiApplOraMetricSvrGet but does not contain the "value", "link" and
...
//
type CmdbCiApplOraMetricSvrPost struct {
	Result struct {
		Attributes struct {
...
// The generateprovidersource utility constructs this STRUCT from metadata pulled from ServiceNow.
//
type CmdbCiApplOraMetricSvrGet struct {
	Result struct {
		Attributes struct {
			TcpPort                     string `json:"tcp_port,omitempty"`
			SkipSync                    string `json:"skip_sync,omitempty"`
...
func ResourceCmdbCiApplOraMetricSvr() *schema.Resource {
	return &schema.Resource{
		Create: createResourceCmdbCiApplOraMetricSvr,
		Read:   readResourceCmdbCiApplOraMetricSvr,
		Update: updateResourceCmdbCiApplOraMetricSvr,
		Delete: deleteResourceCmdbCiApplOraMetricSvr,
...
//
func createResourceCmdbCiApplOraMetricSvr(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client) //Client Connection details
	// Use common function to update base attributes
	var ci CmdbCiApplOraMetricSvrPost
	if err := copyFromTerraformToServiceNowCmdbCiApplOraMetricSvr(resourceData, &ci); err != nil {
		return err
	}
	...
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_appl_ora_metric_svr + "?sysparm_fields=sys_id"
	...
	return readResourceCmdbCiApplOraMetricSvr(resourceData, serviceNowClient)
}

//  Read routine - This function is called when Terraform wants to create a new CI in the ServiceNow CMDB
//
func readResourceCmdbCiApplOraMetricSvr(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client)
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_appl_ora_metric_svr + "/" + resourceData.Id()
	var jsonData []byte
	...
	if err := copyFromServiceNowToTerraformCmdbCiApplOraMetricSvr(resourceData, jsonData); err != nil {
		return err
	}
...
//
func updateResourceCmdbCiApplOraMetricSvr(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	servicenowClient := serviceNowClient.(*Client)
...
//
func deleteResourceCmdbCiApplOraMetricSvr(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	...
	if err := resourceData.Set("install_status", "retired"); err != nil {
		return fmt.Errorf("CmdbCiApplOraMetricSvrfailed to set install_status field during destroy action %s", err)
	}
...
//
func copyFromTerraformToServiceNowCmdbCiApplOraMetricSvr(resourceData *schema.ResourceData, ci *CmdbCiApplOraMetricSvrPost) error {
...
//
func copyFromServiceNowToTerraformCmdbCiApplOraMetricSvr(resourceData *schema.ResourceData, jsonData []byte) error {
	ci := CmdbCiApplOraMetricSvrGet{}
