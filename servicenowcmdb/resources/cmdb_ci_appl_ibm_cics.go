...
//
//  Description       :   This file is the resource provider for the cmdb_ci_appl_ibm_cics CMDB Class.  This code is executed
//                        when the servicenowcmdb_cmdb_ci_appl_ibm_cics keyword is used in a terraform script (*.tf) file
//
//                        This file is will need to be regenerated if the ServiceNow CMDB base CI Class
//                        "cmdb_ci" or of the cmdb_ci_appl_ibm_cics CI Class is modified.
///////////////////////////////////////////////////////////////////////////////////////////////////
import (
	"encoding/json"
	"fmt"
...
const CiNamecmdb_ci_appl_ibm_cics = "cmdb_ci_appl_ibm_cics"
...
// This is essentially the same as CmdbCiApplIbmCicsGet but does not contain the "value", "link" and
...
//
type CmdbCiApplIbmCicsPost struct {
	Result struct {
		Attributes struct {
...
// The generateprovidersource utility constructs this STRUCT from metadata pulled from ServiceNow.
//
type CmdbCiApplIbmCicsGet struct {
	Result struct {
		Attributes struct {
			TcpPort                     string `json:"tcp_port,omitempty"`
			SkipSync                    string `json:"skip_sync,omitempty"`
...
func ResourceCmdbCiApplIbmCics() *schema.Resource {
	return &schema.Resource{
		Create: createResourceCmdbCiApplIbmCics,
		Read:   readResourceCmdbCiApplIbmCics,
		Update: updateResourceCmdbCiApplIbmCics,
		Delete: deleteResourceCmdbCiApplIbmCics,
...
//
func createResourceCmdbCiApplIbmCics(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client) //Client Connection details
	// Use common function to update base attributes
	var ci CmdbCiApplIbmCicsPost
	if err := copyFromTerraformToServiceNowCmdbCiApplIbmCics(resourceData, &ci); err != nil {
		return err
	}
	...
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_appl_ibm_cics + "?sysparm_fields=sys_id"
	...
	return readResourceCmdbCiApplIbmCics(resourceData, serviceNowClient)
}

//  Read routine - This function is called when Terraform wants to create a new CI in the ServiceNow CMDB
//
func readResourceCmdbCiApplIbmCics(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client)
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_appl_ibm_cics + "/" + resourceData.Id()
	var jsonData []byte
	...
	if err := copyFromServiceNowToTerraformCmdbCiApplIbmCics(resourceData, jsonData); err != nil {
		return err
	}
...
//
func updateResourceCmdbCiApplIbmCics(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	servicenowClient := serviceNowClient.(*Client)
...
//
func deleteResourceCmdbCiApplIbmCics(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	...
	if err := resourceData.Set("install_status", "retired"); err != nil {
		return fmt.Errorf("CmdbCiApplIbmCicsfailed to set install_status field during destroy action %s", err)
	}
...
//
func copyFromTerraformToServiceNowCmdbCiApplIbmCics(resourceData *schema.ResourceData, ci *CmdbCiApplIbmCicsPost) error {
...
//
func copyFromServiceNowToTerraformCmdbCiApplIbmCics(resourceData *schema.ResourceData, jsonData []byte) error {
	ci := CmdbCiApplIbmCicsGet{}
