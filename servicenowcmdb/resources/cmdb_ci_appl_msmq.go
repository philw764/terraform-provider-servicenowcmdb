...
//
//  Description       :   This file is the resource provider for the cmdb_ci_appl_msmq CMDB Class.  This code is executed
//                        when the servicenowcmdb_cmdb_ci_appl_msmq keyword is used in a terraform script (*.tf) file
//
//                        This file is will need to be regenerated if the ServiceNow CMDB base CI Class
//                        "cmdb_ci" or of the cmdb_ci_appl_msmq CI Class is modified.
///////////////////////////////////////////////////////////////////////////////////////////////////
import (
	"encoding/json"
	"fmt"
...
const CiNamecmdb_ci_appl_msmq = "cmdb_ci_appl_msmq"
...
// This is essentially the same as CmdbCiApplMsmqGet but does not contain the "value", "link" and
...
//
type CmdbCiApplMsmqPost struct {
	Result struct {
		Attributes struct {
			TcpPort                     string `json:"tcp_port,omitempty"`
			SkipSync                    string `json:"skip_sync,omitempty"`
			OperationalStatus           string `json:"operational_status,omitempty"`
			MsmqMqseriesBridge          string `json:"msmq_mqseries_bridge,omitempty"`
...
func ResourceCmdbCiApplMsmq() *schema.Resource {
	return &schema.Resource{
		Create: createResourceCmdbCiApplMsmq,
		Read:   readResourceCmdbCiApplMsmq,
		Update: updateResourceCmdbCiApplMsmq,
		Delete: deleteResourceCmdbCiApplMsmq,
...
//
func createResourceCmdbCiApplMsmq(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client) //Client Connection details
	// Use common function to update base attributes
	var ci CmdbCiApplMsmqPost
	if err := copyFromTerraformToServiceNowCmdbCiApplMsmq(resourceData, &ci); err != nil {
		return err
	}
	...
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_appl_msmq + "?sysparm_fields=sys_id"
	...
	return readResourceCmdbCiApplMsmq(resourceData, serviceNowClient)
}

//  Read routine - This function is called when Terraform wants to create a new CI in the ServiceNow CMDB
//
func readResourceCmdbCiApplMsmq(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client)
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_appl_msmq + "/" + resourceData.Id()
	var jsonData []byte
	...
	if err := copyFromServiceNowToTerraformCmdbCiApplMsmq(resourceData, jsonData); err != nil {
		return err
	}
...
//
func updateResourceCmdbCiApplMsmq(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	servicenowClient := serviceNowClient.(*Client)
...
//
func deleteResourceCmdbCiApplMsmq(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	...
	if err := resourceData.Set("install_status", "retired"); err != nil {
		return fmt.Errorf("CmdbCiApplMsmqfailed to set install_status field during destroy action %s", err)
	}
...
//
func copyFromTerraformToServiceNowCmdbCiApplMsmq(resourceData *schema.ResourceData, ci *CmdbCiApplMsmqPost) error {
	...
	attrs.MsmqMqseriesBridge = resourceData.Get("msmq_mqseries_bridge").(string)
...
//
func copyFromServiceNowToTerraformCmdbCiApplMsmq(resourceData *schema.ResourceData, jsonData []byte) error {
	ci := CmdbCiApplMsmqGet{}
	...
	_ = resourceData.Set("msmq_mqseries_bridge", attrs.MsmqMqseriesBridge)
