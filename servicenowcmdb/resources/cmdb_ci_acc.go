...
//
//  Description       :   This file is the resource provider for the cmdb_ci_acc CMDB Class.  This code is executed
//                        when the servicenowcmdb_cmdb_ci_acc keyword is used in a terraform script (*.tf) file
//
//                        This file is will need to be regenerated if the ServiceNow CMDB base CI Class
//                        "cmdb_ci" or of the cmdb_ci_acc CI Class is modified.
///////////////////////////////////////////////////////////////////////////////////////////////////
import (
...
const CiNamecmdb_ci_acc  = "cmdb_ci_acc"
...
// This is essentially the same as CmdbCiAccGet but does not contain the "value", "link" and
...
func ResourceCmdbCiAcc() *schema.Resource {
return &schema.Resource{
Create: createResourceCmdbCiAcc,
Read:   readResourceCmdbCiAcc,
Update: updateResourceCmdbCiAcc,
Delete: deleteResourceCmdbCiAcc,
...
//
func createResourceCmdbCiAcc(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

servicenowClient := serviceNowClient.(*Client) //Client Connection details
// Use common function to update base attributes
var ci CmdbCiAccPost
if err := copyFromTerraformToServiceNowCmdbCiAcc(resourceData, &ci); err != nil {
return err
}
...
SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_acc + "?sysparm_fields=sys_id"
...
return readResourceCmdbCiAcc(resourceData, serviceNowClient)
}

//  Read routine - This function is called when Terraform wants to create a new CI in the ServiceNow CMDB
//
func readResourceCmdbCiAcc(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

servicenowClient := serviceNowClient.(*Client)
SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_acc + "/" + resourceData.Id()
var jsonData []byte
...
if err := copyFromServiceNowToTerraformCmdbCiAcc(resourceData,jsonData);  err != nil {
return err
}
...
//
func updateResourceCmdbCiAcc(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
servicenowClient := serviceNowClient.(*Client)
...
//
func deleteResourceCmdbCiAcc(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
...
return fmt.Errorf("CmdbCiAccfailed to set install_status field during destroy action %s", err)
}
...
//
func copyFromTerraformToServiceNowCmdbCiAcc (resourceData *schema.ResourceData, ci *CmdbCiAccPost) error {
...
//
func copyFromServiceNowToTerraformCmdbCiAcc (resourceData *schema.ResourceData, jsonData [] byte) error {
ci := CmdbCiAccGet{}
