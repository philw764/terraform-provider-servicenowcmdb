...
//
//  Description       :   This file is the resource provider for the cmdb_ci_printer CMDB Class.  This code is executed
//                        when the servicenowcmdb_cmdb_ci_printer keyword is used in a terraform script (*.tf) file
//
//                        This file is will need to be regenerated if the ServiceNow CMDB base CI Class
//                        "cmdb_ci" or of the cmdb_ci_printer CI Class is modified.
///////////////////////////////////////////////////////////////////////////////////////////////////
import (
	"encoding/json"
	"fmt"
...
const CiNamecmdb_ci_printer = "cmdb_ci_printer"
...
// This is essentially the same as CmdbCiPrinterGet but does not contain the "value", "link" and
...
func ResourceCmdbCiPrinter() *schema.Resource {
	return &schema.Resource{
		Create: createResourceCmdbCiPrinter,
		Read:   readResourceCmdbCiPrinter,
		Update: updateResourceCmdbCiPrinter,
		Delete: deleteResourceCmdbCiPrinter,
...
//
func createResourceCmdbCiPrinter(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client) //Client Connection details
	// Use common function to update base attributes
	var ci CmdbCiPrinterPost
	if err := copyFromTerraformToServiceNowCmdbCiPrinter(resourceData, &ci); err != nil {
		return err
	}
	...
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_printer + "?sysparm_fields=sys_id"
	...
	return readResourceCmdbCiPrinter(resourceData, serviceNowClient)
}

//  Read routine - This function is called when Terraform wants to create a new CI in the ServiceNow CMDB
//
func readResourceCmdbCiPrinter(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client)
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_printer + "/" + resourceData.Id()
	var jsonData []byte
	...
	if err := copyFromServiceNowToTerraformCmdbCiPrinter(resourceData, jsonData); err != nil {
		return err
	}
...
//
func updateResourceCmdbCiPrinter(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	servicenowClient := serviceNowClient.(*Client)
...
//
func deleteResourceCmdbCiPrinter(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	...
	if err := resourceData.Set("install_status", "retired"); err != nil {
		return fmt.Errorf("CmdbCiPrinterfailed to set install_status field during destroy action %s", err)
	}
...
//
func copyFromTerraformToServiceNowCmdbCiPrinter(resourceData *schema.ResourceData, ci *CmdbCiPrinterPost) error {
...
//
func copyFromServiceNowToTerraformCmdbCiPrinter(resourceData *schema.ResourceData, jsonData []byte) error {
	ci := CmdbCiPrinterGet{}
