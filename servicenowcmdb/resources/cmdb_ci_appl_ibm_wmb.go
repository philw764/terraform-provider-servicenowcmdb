...
//
//  Description       :   This file is the resource provider for the cmdb_ci_appl_ibm_wmb CMDB Class.  This code is executed
//                        when the servicenowcmdb_cmdb_ci_appl_ibm_wmb keyword is used in a terraform script (*.tf) file
//
//                        This file is will need to be regenerated if the ServiceNow CMDB base CI Class
//                        "cmdb_ci" or of the cmdb_ci_appl_ibm_wmb CI Class is modified.
///////////////////////////////////////////////////////////////////////////////////////////////////
import (
	"encoding/json"
	"fmt"
...
const CiNamecmdb_ci_appl_ibm_wmb = "cmdb_ci_appl_ibm_wmb"
...
// This is essentially the same as CmdbCiApplIbmWmbGet but does not contain the "value", "link" and
...
//
type CmdbCiApplIbmWmbPost struct {
	Result struct {
		Attributes struct {
...
// The generateprovidersource utility constructs this STRUCT from metadata pulled from ServiceNow.
//
type CmdbCiApplIbmWmbGet struct {
	Result struct {
		Attributes struct {
			TcpPort                     string `json:"tcp_port,omitempty"`
			SkipSync                    string `json:"skip_sync,omitempty"`
...
func ResourceCmdbCiApplIbmWmb() *schema.Resource {
	return &schema.Resource{
		Create: createResourceCmdbCiApplIbmWmb,
		Read:   readResourceCmdbCiApplIbmWmb,
		Update: updateResourceCmdbCiApplIbmWmb,
		Delete: deleteResourceCmdbCiApplIbmWmb,
...
//
func createResourceCmdbCiApplIbmWmb(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client) //Client Connection details
	// Use common function to update base attributes
	var ci CmdbCiApplIbmWmbPost
	if err := copyFromTerraformToServiceNowCmdbCiApplIbmWmb(resourceData, &ci); err != nil {
		return err
	}
	...
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_appl_ibm_wmb + "?sysparm_fields=sys_id"
	...
	return readResourceCmdbCiApplIbmWmb(resourceData, serviceNowClient)
}

//  Read routine - This function is called when Terraform wants to create a new CI in the ServiceNow CMDB
//
func readResourceCmdbCiApplIbmWmb(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client)
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_appl_ibm_wmb + "/" + resourceData.Id()
	var jsonData []byte
	...
	if err := copyFromServiceNowToTerraformCmdbCiApplIbmWmb(resourceData, jsonData); err != nil {
		return err
	}
...
//
func updateResourceCmdbCiApplIbmWmb(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	servicenowClient := serviceNowClient.(*Client)
...
//
func deleteResourceCmdbCiApplIbmWmb(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	...
	if err := resourceData.Set("install_status", "retired"); err != nil {
		return fmt.Errorf("CmdbCiApplIbmWmbfailed to set install_status field during destroy action %s", err)
	}
...
//
func copyFromTerraformToServiceNowCmdbCiApplIbmWmb(resourceData *schema.ResourceData, ci *CmdbCiApplIbmWmbPost) error {
...
//
func copyFromServiceNowToTerraformCmdbCiApplIbmWmb(resourceData *schema.ResourceData, jsonData []byte) error {
	ci := CmdbCiApplIbmWmbGet{}
