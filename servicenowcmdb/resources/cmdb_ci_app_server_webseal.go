...
//
//  Description       :   This file is the resource provider for the cmdb_ci_app_server_webseal CMDB Class.  This code is executed
//                        when the servicenowcmdb_cmdb_ci_app_server_webseal keyword is used in a terraform script (*.tf) file
//
//                        This file is will need to be regenerated if the ServiceNow CMDB base CI Class
//                        "cmdb_ci" or of the cmdb_ci_app_server_webseal CI Class is modified.
///////////////////////////////////////////////////////////////////////////////////////////////////
import (
	"encoding/json"
	"fmt"
...
const CiNamecmdb_ci_app_server_webseal = "cmdb_ci_app_server_webseal"
...
// This is essentially the same as CmdbCiAppServerWebsealGet but does not contain the "value", "link" and
...
//
type CmdbCiAppServerWebsealPost struct {
	Result struct {
		Attributes struct {
...
// The generateprovidersource utility constructs this STRUCT from metadata pulled from ServiceNow.
//
type CmdbCiAppServerWebsealGet struct {
	Result struct {
		Attributes struct {
			TcpPort                     string `json:"tcp_port,omitempty"`
			SkipSync                    string `json:"skip_sync,omitempty"`
...
func ResourceCmdbCiAppServerWebseal() *schema.Resource {
	return &schema.Resource{
		Create: createResourceCmdbCiAppServerWebseal,
		Read:   readResourceCmdbCiAppServerWebseal,
		Update: updateResourceCmdbCiAppServerWebseal,
		Delete: deleteResourceCmdbCiAppServerWebseal,
...
//
func createResourceCmdbCiAppServerWebseal(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client) //Client Connection details
	// Use common function to update base attributes
	var ci CmdbCiAppServerWebsealPost
	if err := copyFromTerraformToServiceNowCmdbCiAppServerWebseal(resourceData, &ci); err != nil {
		return err
	}
	...
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_app_server_webseal + "?sysparm_fields=sys_id"
	...
	return readResourceCmdbCiAppServerWebseal(resourceData, serviceNowClient)
}

//  Read routine - This function is called when Terraform wants to create a new CI in the ServiceNow CMDB
//
func readResourceCmdbCiAppServerWebseal(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client)
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_app_server_webseal + "/" + resourceData.Id()
	var jsonData []byte
	...
	if err := copyFromServiceNowToTerraformCmdbCiAppServerWebseal(resourceData, jsonData); err != nil {
		return err
	}
...
//
func updateResourceCmdbCiAppServerWebseal(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	servicenowClient := serviceNowClient.(*Client)
...
//
func deleteResourceCmdbCiAppServerWebseal(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	...
	if err := resourceData.Set("install_status", "retired"); err != nil {
		return fmt.Errorf("CmdbCiAppServerWebsealfailed to set install_status field during destroy action %s", err)
	}
...
//
func copyFromTerraformToServiceNowCmdbCiAppServerWebseal(resourceData *schema.ResourceData, ci *CmdbCiAppServerWebsealPost) error {
...
//
func copyFromServiceNowToTerraformCmdbCiAppServerWebseal(resourceData *schema.ResourceData, jsonData []byte) error {
	ci := CmdbCiAppServerWebsealGet{}
