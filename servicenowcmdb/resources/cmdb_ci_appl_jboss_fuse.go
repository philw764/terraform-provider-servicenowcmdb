...
//
//  Description       :   This file is the resource provider for the cmdb_ci_appl_jboss_fuse CMDB Class.  This code is executed
//                        when the servicenowcmdb_cmdb_ci_appl_jboss_fuse keyword is used in a terraform script (*.tf) file
//
//                        This file is will need to be regenerated if the ServiceNow CMDB base CI Class
//                        "cmdb_ci" or of the cmdb_ci_appl_jboss_fuse CI Class is modified.
///////////////////////////////////////////////////////////////////////////////////////////////////
import (
	"encoding/json"
	"fmt"
...
const CiNamecmdb_ci_appl_jboss_fuse = "cmdb_ci_appl_jboss_fuse"
...
// This is essentially the same as CmdbCiApplJbossFuseGet but does not contain the "value", "link" and
...
//
type CmdbCiApplJbossFusePost struct {
	Result struct {
		Attributes struct {
...
// The generateprovidersource utility constructs this STRUCT from metadata pulled from ServiceNow.
//
type CmdbCiApplJbossFuseGet struct {
	Result struct {
		Attributes struct {
			TcpPort                     string `json:"tcp_port,omitempty"`
			SkipSync                    string `json:"skip_sync,omitempty"`
...
func ResourceCmdbCiApplJbossFuse() *schema.Resource {
	return &schema.Resource{
		Create: createResourceCmdbCiApplJbossFuse,
		Read:   readResourceCmdbCiApplJbossFuse,
		Update: updateResourceCmdbCiApplJbossFuse,
		Delete: deleteResourceCmdbCiApplJbossFuse,
...
//
func createResourceCmdbCiApplJbossFuse(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client) //Client Connection details
	// Use common function to update base attributes
	var ci CmdbCiApplJbossFusePost
	if err := copyFromTerraformToServiceNowCmdbCiApplJbossFuse(resourceData, &ci); err != nil {
		return err
	}
	...
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_appl_jboss_fuse + "?sysparm_fields=sys_id"
	...
	return readResourceCmdbCiApplJbossFuse(resourceData, serviceNowClient)
}

//  Read routine - This function is called when Terraform wants to create a new CI in the ServiceNow CMDB
//
func readResourceCmdbCiApplJbossFuse(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client)
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_appl_jboss_fuse + "/" + resourceData.Id()
	var jsonData []byte
	...
	if err := copyFromServiceNowToTerraformCmdbCiApplJbossFuse(resourceData, jsonData); err != nil {
		return err
	}
...
//
func updateResourceCmdbCiApplJbossFuse(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	servicenowClient := serviceNowClient.(*Client)
...
//
func deleteResourceCmdbCiApplJbossFuse(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	...
	if err := resourceData.Set("install_status", "retired"); err != nil {
		return fmt.Errorf("CmdbCiApplJbossFusefailed to set install_status field during destroy action %s", err)
	}
...
//
func copyFromTerraformToServiceNowCmdbCiApplJbossFuse(resourceData *schema.ResourceData, ci *CmdbCiApplJbossFusePost) error {
...
//
func copyFromServiceNowToTerraformCmdbCiApplJbossFuse(resourceData *schema.ResourceData, jsonData []byte) error {
	ci := CmdbCiApplJbossFuseGet{}
