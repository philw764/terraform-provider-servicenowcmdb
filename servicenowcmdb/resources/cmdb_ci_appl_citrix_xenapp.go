...
//
//  Description       :   This file is the resource provider for the cmdb_ci_appl_citrix_xenapp CMDB Class.  This code is executed
//                        when the servicenowcmdb_cmdb_ci_appl_citrix_xenapp keyword is used in a terraform script (*.tf) file
//
//                        This file is will need to be regenerated if the ServiceNow CMDB base CI Class
//                        "cmdb_ci" or of the cmdb_ci_appl_citrix_xenapp CI Class is modified.
///////////////////////////////////////////////////////////////////////////////////////////////////
import (
	"encoding/json"
	"fmt"
...
const CiNamecmdb_ci_appl_citrix_xenapp = "cmdb_ci_appl_citrix_xenapp"
...
// This is essentially the same as CmdbCiApplCitrixXenappGet but does not contain the "value", "link" and
...
//
type CmdbCiApplCitrixXenappPost struct {
	Result struct {
		Attributes struct {
...
// The generateprovidersource utility constructs this STRUCT from metadata pulled from ServiceNow.
//
type CmdbCiApplCitrixXenappGet struct {
	Result struct {
		Attributes struct {
			TcpPort                     string `json:"tcp_port,omitempty"`
			SkipSync                    string `json:"skip_sync,omitempty"`
...
func ResourceCmdbCiApplCitrixXenapp() *schema.Resource {
	return &schema.Resource{
		Create: createResourceCmdbCiApplCitrixXenapp,
		Read:   readResourceCmdbCiApplCitrixXenapp,
		Update: updateResourceCmdbCiApplCitrixXenapp,
		Delete: deleteResourceCmdbCiApplCitrixXenapp,
...
//
func createResourceCmdbCiApplCitrixXenapp(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client) //Client Connection details
	// Use common function to update base attributes
	var ci CmdbCiApplCitrixXenappPost
	if err := copyFromTerraformToServiceNowCmdbCiApplCitrixXenapp(resourceData, &ci); err != nil {
		return err
	}
	...
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_appl_citrix_xenapp + "?sysparm_fields=sys_id"
	...
	return readResourceCmdbCiApplCitrixXenapp(resourceData, serviceNowClient)
}

//  Read routine - This function is called when Terraform wants to create a new CI in the ServiceNow CMDB
//
func readResourceCmdbCiApplCitrixXenapp(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client)
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_appl_citrix_xenapp + "/" + resourceData.Id()
	var jsonData []byte
	...
	if err := copyFromServiceNowToTerraformCmdbCiApplCitrixXenapp(resourceData, jsonData); err != nil {
		return err
	}
...
//
func updateResourceCmdbCiApplCitrixXenapp(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	servicenowClient := serviceNowClient.(*Client)
...
//
func deleteResourceCmdbCiApplCitrixXenapp(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	...
	if err := resourceData.Set("install_status", "retired"); err != nil {
		return fmt.Errorf("CmdbCiApplCitrixXenappfailed to set install_status field during destroy action %s", err)
	}
...
//
func copyFromTerraformToServiceNowCmdbCiApplCitrixXenapp(resourceData *schema.ResourceData, ci *CmdbCiApplCitrixXenappPost) error {
...
//
func copyFromServiceNowToTerraformCmdbCiApplCitrixXenapp(resourceData *schema.ResourceData, jsonData []byte) error {
	ci := CmdbCiApplCitrixXenappGet{}
