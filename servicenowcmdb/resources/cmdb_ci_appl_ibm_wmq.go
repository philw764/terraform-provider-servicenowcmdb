...
//
//  Description       :   This file is the resource provider for the cmdb_ci_appl_ibm_wmq CMDB Class.  This code is executed
//                        when the servicenowcmdb_cmdb_ci_appl_ibm_wmq keyword is used in a terraform script (*.tf) file
//
//                        This file is will need to be regenerated if the ServiceNow CMDB base CI Class
//                        "cmdb_ci" or of the cmdb_ci_appl_ibm_wmq CI Class is modified.
///////////////////////////////////////////////////////////////////////////////////////////////////
import (
	"encoding/json"
	"fmt"
...
const CiNamecmdb_ci_appl_ibm_wmq = "cmdb_ci_appl_ibm_wmq"
...
// This is essentially the same as CmdbCiApplIbmWmqGet but does not contain the "value", "link" and
...
//
type CmdbCiApplIbmWmqPost struct {
	Result struct {
		Attributes struct {
...
// The generateprovidersource utility constructs this STRUCT from metadata pulled from ServiceNow.
//
type CmdbCiApplIbmWmqGet struct {
	Result struct {
		Attributes struct {
			TcpPort                     string `json:"tcp_port,omitempty"`
			SkipSync                    string `json:"skip_sync,omitempty"`
...
func ResourceCmdbCiApplIbmWmq() *schema.Resource {
	return &schema.Resource{
		Create: createResourceCmdbCiApplIbmWmq,
		Read:   readResourceCmdbCiApplIbmWmq,
		Update: updateResourceCmdbCiApplIbmWmq,
		Delete: deleteResourceCmdbCiApplIbmWmq,
...
//
func createResourceCmdbCiApplIbmWmq(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client) //Client Connection details
	// Use common function to update base attributes
	var ci CmdbCiApplIbmWmqPost
	if err := copyFromTerraformToServiceNowCmdbCiApplIbmWmq(resourceData, &ci); err != nil {
		return err
	}
	...
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_appl_ibm_wmq + "?sysparm_fields=sys_id"
	...
	return readResourceCmdbCiApplIbmWmq(resourceData, serviceNowClient)
}

//  Read routine - This function is called when Terraform wants to create a new CI in the ServiceNow CMDB
//
func readResourceCmdbCiApplIbmWmq(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client)
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_appl_ibm_wmq + "/" + resourceData.Id()
	var jsonData []byte
	...
	if err := copyFromServiceNowToTerraformCmdbCiApplIbmWmq(resourceData, jsonData); err != nil {
		return err
	}
...
//
func updateResourceCmdbCiApplIbmWmq(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	servicenowClient := serviceNowClient.(*Client)
...
//
func deleteResourceCmdbCiApplIbmWmq(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	...
	if err := resourceData.Set("install_status", "retired"); err != nil {
		return fmt.Errorf("CmdbCiApplIbmWmqfailed to set install_status field during destroy action %s", err)
	}
...
//
func copyFromTerraformToServiceNowCmdbCiApplIbmWmq(resourceData *schema.ResourceData, ci *CmdbCiApplIbmWmqPost) error {
...
//
func copyFromServiceNowToTerraformCmdbCiApplIbmWmq(resourceData *schema.ResourceData, jsonData []byte) error {
	ci := CmdbCiApplIbmWmqGet{}
