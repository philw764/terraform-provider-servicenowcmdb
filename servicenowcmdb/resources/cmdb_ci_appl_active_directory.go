...
//
//  Description       :   This file is the resource provider for the cmdb_ci_appl_active_directory CMDB Class.  This code is executed
//                        when the servicenowcmdb_cmdb_ci_appl_active_directory keyword is used in a terraform script (*.tf) file
//
//                        This file is will need to be regenerated if the ServiceNow CMDB base CI Class
//                        "cmdb_ci" or of the cmdb_ci_appl_active_directory CI Class is modified.
///////////////////////////////////////////////////////////////////////////////////////////////////
import (
	"encoding/json"
	"fmt"
...
const CiNamecmdb_ci_appl_active_directory = "cmdb_ci_appl_active_directory"
...
// This is essentially the same as CmdbCiApplActiveDirectoryGet but does not contain the "value", "link" and
...
//
type CmdbCiApplActiveDirectoryPost struct {
	Result struct {
		Attributes struct {
...
// The generateprovidersource utility constructs this STRUCT from metadata pulled from ServiceNow.
//
type CmdbCiApplActiveDirectoryGet struct {
	Result struct {
		Attributes struct {
			TcpPort                     string `json:"tcp_port,omitempty"`
			SkipSync                    string `json:"skip_sync,omitempty"`
...
func ResourceCmdbCiApplActiveDirectory() *schema.Resource {
	return &schema.Resource{
		Create: createResourceCmdbCiApplActiveDirectory,
		Read:   readResourceCmdbCiApplActiveDirectory,
		Update: updateResourceCmdbCiApplActiveDirectory,
		Delete: deleteResourceCmdbCiApplActiveDirectory,
...
//
func createResourceCmdbCiApplActiveDirectory(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client) //Client Connection details
	// Use common function to update base attributes
	var ci CmdbCiApplActiveDirectoryPost
	if err := copyFromTerraformToServiceNowCmdbCiApplActiveDirectory(resourceData, &ci); err != nil {
		return err
	}
	...
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_appl_active_directory + "?sysparm_fields=sys_id"
	...
	return readResourceCmdbCiApplActiveDirectory(resourceData, serviceNowClient)
}

//  Read routine - This function is called when Terraform wants to create a new CI in the ServiceNow CMDB
//
func readResourceCmdbCiApplActiveDirectory(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client)
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_appl_active_directory + "/" + resourceData.Id()
	var jsonData []byte
	...
	if err := copyFromServiceNowToTerraformCmdbCiApplActiveDirectory(resourceData, jsonData); err != nil {
		return err
	}
...
//
func updateResourceCmdbCiApplActiveDirectory(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	servicenowClient := serviceNowClient.(*Client)
...
//
func deleteResourceCmdbCiApplActiveDirectory(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	...
	if err := resourceData.Set("install_status", "retired"); err != nil {
		return fmt.Errorf("CmdbCiApplActiveDirectoryfailed to set install_status field during destroy action %s", err)
	}
...
//
func copyFromTerraformToServiceNowCmdbCiApplActiveDirectory(resourceData *schema.ResourceData, ci *CmdbCiApplActiveDirectoryPost) error {
...
//
func copyFromServiceNowToTerraformCmdbCiApplActiveDirectory(resourceData *schema.ResourceData, jsonData []byte) error {
	ci := CmdbCiApplActiveDirectoryGet{}
