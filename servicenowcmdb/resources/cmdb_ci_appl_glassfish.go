...
//
//  Description       :   This file is the resource provider for the cmdb_ci_appl_glassfish CMDB Class.  This code is executed
//                        when the servicenowcmdb_cmdb_ci_appl_glassfish keyword is used in a terraform script (*.tf) file
//
//                        This file is will need to be regenerated if the ServiceNow CMDB base CI Class
//                        "cmdb_ci" or of the cmdb_ci_appl_glassfish CI Class is modified.
///////////////////////////////////////////////////////////////////////////////////////////////////
import (
	"encoding/json"
	"fmt"
...
const CiNamecmdb_ci_appl_glassfish = "cmdb_ci_appl_glassfish"
...
// This is essentially the same as CmdbCiApplGlassfishGet but does not contain the "value", "link" and
...
//
type CmdbCiApplGlassfishPost struct {
	Result struct {
		Attributes struct {
...
// The generateprovidersource utility constructs this STRUCT from metadata pulled from ServiceNow.
//
type CmdbCiApplGlassfishGet struct {
	Result struct {
		Attributes struct {
			Container                   string `json:"container,omitempty"`
			TcpPort                     string `json:"tcp_port,omitempty"`
...
func ResourceCmdbCiApplGlassfish() *schema.Resource {
	return &schema.Resource{
		Create: createResourceCmdbCiApplGlassfish,
		Read:   readResourceCmdbCiApplGlassfish,
		Update: updateResourceCmdbCiApplGlassfish,
		Delete: deleteResourceCmdbCiApplGlassfish,
...
//
func createResourceCmdbCiApplGlassfish(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client) //Client Connection details
	// Use common function to update base attributes
	var ci CmdbCiApplGlassfishPost
	if err := copyFromTerraformToServiceNowCmdbCiApplGlassfish(resourceData, &ci); err != nil {
		return err
	}
	...
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_appl_glassfish + "?sysparm_fields=sys_id"
	...
	return readResourceCmdbCiApplGlassfish(resourceData, serviceNowClient)
}

//  Read routine - This function is called when Terraform wants to create a new CI in the ServiceNow CMDB
//
func readResourceCmdbCiApplGlassfish(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client)
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_appl_glassfish + "/" + resourceData.Id()
	var jsonData []byte
	...
	if err := copyFromServiceNowToTerraformCmdbCiApplGlassfish(resourceData, jsonData); err != nil {
		return err
	}
...
//
func updateResourceCmdbCiApplGlassfish(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	servicenowClient := serviceNowClient.(*Client)
...
//
func deleteResourceCmdbCiApplGlassfish(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	...
	if err := resourceData.Set("install_status", "retired"); err != nil {
		return fmt.Errorf("CmdbCiApplGlassfishfailed to set install_status field during destroy action %s", err)
	}
...
//
func copyFromTerraformToServiceNowCmdbCiApplGlassfish(resourceData *schema.ResourceData, ci *CmdbCiApplGlassfishPost) error {
	...
	attrs.MatchUrl = resourceData.Get("match_url").(string)
	...
	attrs.InstName = resourceData.Get("inst_name").(string)
...
//
func copyFromServiceNowToTerraformCmdbCiApplGlassfish(resourceData *schema.ResourceData, jsonData []byte) error {
	ci := CmdbCiApplGlassfishGet{}
	...
	_ = resourceData.Set("match_url", attrs.MatchUrl)
	...
	_ = resourceData.Set("inst_name", attrs.InstName)
