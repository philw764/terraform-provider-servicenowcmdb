...
//
//  Description       :   This file is the resource provider for the cmdb_ci_appl_apigee_srv CMDB Class.  This code is executed
//                        when the servicenowcmdb_cmdb_ci_appl_apigee_srv keyword is used in a terraform script (*.tf) file
//
//                        This file is will need to be regenerated if the ServiceNow CMDB base CI Class
//                        "cmdb_ci" or of the cmdb_ci_appl_apigee_srv CI Class is modified.
///////////////////////////////////////////////////////////////////////////////////////////////////
import (
	"encoding/json"
	"fmt"
...
const CiNamecmdb_ci_appl_apigee_srv = "cmdb_ci_appl_apigee_srv"
...
// This is essentially the same as CmdbCiApplApigeeSrvGet but does not contain the "value", "link" and
...
//
type CmdbCiApplApigeeSrvPost struct {
	Result struct {
		Attributes struct {
...
// The generateprovidersource utility constructs this STRUCT from metadata pulled from ServiceNow.
//
type CmdbCiApplApigeeSrvGet struct {
	Result struct {
		Attributes struct {
			TcpPort                     string `json:"tcp_port,omitempty"`
			SkipSync                    string `json:"skip_sync,omitempty"`
...
func ResourceCmdbCiApplApigeeSrv() *schema.Resource {
	return &schema.Resource{
		Create: createResourceCmdbCiApplApigeeSrv,
		Read:   readResourceCmdbCiApplApigeeSrv,
		Update: updateResourceCmdbCiApplApigeeSrv,
		Delete: deleteResourceCmdbCiApplApigeeSrv,
...
//
func createResourceCmdbCiApplApigeeSrv(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client) //Client Connection details
	// Use common function to update base attributes
	var ci CmdbCiApplApigeeSrvPost
	if err := copyFromTerraformToServiceNowCmdbCiApplApigeeSrv(resourceData, &ci); err != nil {
		return err
	}
	...
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_appl_apigee_srv + "?sysparm_fields=sys_id"
	...
	return readResourceCmdbCiApplApigeeSrv(resourceData, serviceNowClient)
}

//  Read routine - This function is called when Terraform wants to create a new CI in the ServiceNow CMDB
//
func readResourceCmdbCiApplApigeeSrv(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client)
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_appl_apigee_srv + "/" + resourceData.Id()
	var jsonData []byte
	...
	if err := copyFromServiceNowToTerraformCmdbCiApplApigeeSrv(resourceData, jsonData); err != nil {
		return err
	}
...
//
func updateResourceCmdbCiApplApigeeSrv(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	servicenowClient := serviceNowClient.(*Client)
...
//
func deleteResourceCmdbCiApplApigeeSrv(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	...
	if err := resourceData.Set("install_status", "retired"); err != nil {
		return fmt.Errorf("CmdbCiApplApigeeSrvfailed to set install_status field during destroy action %s", err)
	}
...
//
func copyFromTerraformToServiceNowCmdbCiApplApigeeSrv(resourceData *schema.ResourceData, ci *CmdbCiApplApigeeSrvPost) error {
...
//
func copyFromServiceNowToTerraformCmdbCiApplApigeeSrv(resourceData *schema.ResourceData, jsonData []byte) error {
	ci := CmdbCiApplApigeeSrvGet{}
