...
//
//  Description       :   This file is the resource provider for the cmdb_ci_app_server_wl_module CMDB Class.  This code is executed
//                        when the servicenowcmdb_cmdb_ci_app_server_wl_module keyword is used in a terraform script (*.tf) file
//
//                        This file is will need to be regenerated if the ServiceNow CMDB base CI Class
//                        "cmdb_ci" or of the cmdb_ci_app_server_wl_module CI Class is modified.
///////////////////////////////////////////////////////////////////////////////////////////////////
import (
	"encoding/json"
	"fmt"
...
const CiNamecmdb_ci_app_server_wl_module = "cmdb_ci_app_server_wl_module"
...
// This is essentially the same as CmdbCiAppServerWlModuleGet but does not contain the "value", "link" and
...
//
type CmdbCiAppServerWlModulePost struct {
	Result struct {
		Attributes struct {
			...
			AppTmpdir                   string `json:"app_tmpdir,omitempty"`
			...
			BeaFile                     string `json:"bea_file,omitempty"`
...
// The generateprovidersource utility constructs this STRUCT from metadata pulled from ServiceNow.
//
type CmdbCiAppServerWlModuleGet struct {
	Result struct {
		Attributes struct {
			Container                   string `json:"container,omitempty"`
			TcpPort                     string `json:"tcp_port,omitempty"`
...
func ResourceCmdbCiAppServerWlModule() *schema.Resource {
	return &schema.Resource{
		Create: createResourceCmdbCiAppServerWlModule,
		Read:   readResourceCmdbCiAppServerWlModule,
		Update: updateResourceCmdbCiAppServerWlModule,
		Delete: deleteResourceCmdbCiAppServerWlModule,
...
//
func createResourceCmdbCiAppServerWlModule(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client) //Client Connection details
	// Use common function to update base attributes
	var ci CmdbCiAppServerWlModulePost
	if err := copyFromTerraformToServiceNowCmdbCiAppServerWlModule(resourceData, &ci); err != nil {
		return err
	}
	...
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_app_server_wl_module + "?sysparm_fields=sys_id"
	...
	return readResourceCmdbCiAppServerWlModule(resourceData, serviceNowClient)
}

//  Read routine - This function is called when Terraform wants to create a new CI in the ServiceNow CMDB
//
func readResourceCmdbCiAppServerWlModule(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client)
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_app_server_wl_module + "/" + resourceData.Id()
	var jsonData []byte
	...
	if err := copyFromServiceNowToTerraformCmdbCiAppServerWlModule(resourceData, jsonData); err != nil {
		return err
	}
...
//
func updateResourceCmdbCiAppServerWlModule(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	servicenowClient := serviceNowClient.(*Client)
...
//
func deleteResourceCmdbCiAppServerWlModule(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	...
	if err := resourceData.Set("install_status", "retired"); err != nil {
		return fmt.Errorf("CmdbCiAppServerWlModulefailed to set install_status field during destroy action %s", err)
	}
...
//
func copyFromTerraformToServiceNowCmdbCiAppServerWlModule(resourceData *schema.ResourceData, ci *CmdbCiAppServerWlModulePost) error {
	...
	attrs.AppTmpdir = resourceData.Get("app_tmpdir").(string)
	...
	attrs.BeaFile = resourceData.Get("bea_file").(string)
	...
	attrs.AppUri = resourceData.Get("app_uri").(string)
...
//
func copyFromServiceNowToTerraformCmdbCiAppServerWlModule(resourceData *schema.ResourceData, jsonData []byte) error {
	ci := CmdbCiAppServerWlModuleGet{}
	...
	_ = resourceData.Set("app_tmpdir", attrs.AppTmpdir)
	...
	_ = resourceData.Set("bea_file", attrs.BeaFile)
	...
	_ = resourceData.Set("app_uri", attrs.AppUri)
