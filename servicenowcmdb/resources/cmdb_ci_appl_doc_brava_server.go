...
//
//  Description       :   This file is the resource provider for the cmdb_ci_appl_doc_brava_server CMDB Class.  This code is executed
//                        when the servicenowcmdb_cmdb_ci_appl_doc_brava_server keyword is used in a terraform script (*.tf) file
//
//                        This file is will need to be regenerated if the ServiceNow CMDB base CI Class
//                        "cmdb_ci" or of the cmdb_ci_appl_doc_brava_server CI Class is modified.
///////////////////////////////////////////////////////////////////////////////////////////////////
import (
	"encoding/json"
	"fmt"
...
const CiNamecmdb_ci_appl_doc_brava_server = "cmdb_ci_appl_doc_brava_server"
...
// This is essentially the same as CmdbCiApplDocBravaServerGet but does not contain the "value", "link" and
...
//
type CmdbCiApplDocBravaServerPost struct {
	Result struct {
		Attributes struct {
...
// The generateprovidersource utility constructs this STRUCT from metadata pulled from ServiceNow.
//
type CmdbCiApplDocBravaServerGet struct {
	Result struct {
		Attributes struct {
			TcpPort                     string `json:"tcp_port,omitempty"`
			SkipSync                    string `json:"skip_sync,omitempty"`
...
func ResourceCmdbCiApplDocBravaServer() *schema.Resource {
	return &schema.Resource{
		Create: createResourceCmdbCiApplDocBravaServer,
		Read:   readResourceCmdbCiApplDocBravaServer,
		Update: updateResourceCmdbCiApplDocBravaServer,
		Delete: deleteResourceCmdbCiApplDocBravaServer,
...
//
func createResourceCmdbCiApplDocBravaServer(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client) //Client Connection details
	// Use common function to update base attributes
	var ci CmdbCiApplDocBravaServerPost
	if err := copyFromTerraformToServiceNowCmdbCiApplDocBravaServer(resourceData, &ci); err != nil {
		return err
	}
	...
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_appl_doc_brava_server + "?sysparm_fields=sys_id"
	...
	return readResourceCmdbCiApplDocBravaServer(resourceData, serviceNowClient)
}

//  Read routine - This function is called when Terraform wants to create a new CI in the ServiceNow CMDB
//
func readResourceCmdbCiApplDocBravaServer(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client)
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_appl_doc_brava_server + "/" + resourceData.Id()
	var jsonData []byte
	...
	if err := copyFromServiceNowToTerraformCmdbCiApplDocBravaServer(resourceData, jsonData); err != nil {
		return err
	}
...
//
func updateResourceCmdbCiApplDocBravaServer(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	servicenowClient := serviceNowClient.(*Client)
...
//
func deleteResourceCmdbCiApplDocBravaServer(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	...
	if err := resourceData.Set("install_status", "retired"); err != nil {
		return fmt.Errorf("CmdbCiApplDocBravaServerfailed to set install_status field during destroy action %s", err)
	}
...
//
func copyFromTerraformToServiceNowCmdbCiApplDocBravaServer(resourceData *schema.ResourceData, ci *CmdbCiApplDocBravaServerPost) error {
...
//
func copyFromServiceNowToTerraformCmdbCiApplDocBravaServer(resourceData *schema.ResourceData, jsonData []byte) error {
	ci := CmdbCiApplDocBravaServerGet{}
