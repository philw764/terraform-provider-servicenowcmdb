...
//
//  Description       :   This file is the resource provider for the cmdb_ci_appl_hp_index CMDB Class.  This code is executed
//                        when the servicenowcmdb_cmdb_ci_appl_hp_index keyword is used in a terraform script (*.tf) file
//
//                        This file is will need to be regenerated if the ServiceNow CMDB base CI Class
//                        "cmdb_ci" or of the cmdb_ci_appl_hp_index CI Class is modified.
///////////////////////////////////////////////////////////////////////////////////////////////////
import (
	"encoding/json"
	"fmt"
...
const CiNamecmdb_ci_appl_hp_index = "cmdb_ci_appl_hp_index"
...
// This is essentially the same as CmdbCiApplHpIndexGet but does not contain the "value", "link" and
...
//
type CmdbCiApplHpIndexPost struct {
	Result struct {
		Attributes struct {
...
// The generateprovidersource utility constructs this STRUCT from metadata pulled from ServiceNow.
//
type CmdbCiApplHpIndexGet struct {
	Result struct {
		Attributes struct {
			TcpPort                     string `json:"tcp_port,omitempty"`
			SkipSync                    string `json:"skip_sync,omitempty"`
...
func ResourceCmdbCiApplHpIndex() *schema.Resource {
	return &schema.Resource{
		Create: createResourceCmdbCiApplHpIndex,
		Read:   readResourceCmdbCiApplHpIndex,
		Update: updateResourceCmdbCiApplHpIndex,
		Delete: deleteResourceCmdbCiApplHpIndex,
...
//
func createResourceCmdbCiApplHpIndex(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client) //Client Connection details
	// Use common function to update base attributes
	var ci CmdbCiApplHpIndexPost
	if err := copyFromTerraformToServiceNowCmdbCiApplHpIndex(resourceData, &ci); err != nil {
		return err
	}
	...
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_appl_hp_index + "?sysparm_fields=sys_id"
	...
	return readResourceCmdbCiApplHpIndex(resourceData, serviceNowClient)
}

//  Read routine - This function is called when Terraform wants to create a new CI in the ServiceNow CMDB
//
func readResourceCmdbCiApplHpIndex(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client)
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_appl_hp_index + "/" + resourceData.Id()
	var jsonData []byte
	...
	if err := copyFromServiceNowToTerraformCmdbCiApplHpIndex(resourceData, jsonData); err != nil {
		return err
	}
...
//
func updateResourceCmdbCiApplHpIndex(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	servicenowClient := serviceNowClient.(*Client)
...
//
func deleteResourceCmdbCiApplHpIndex(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	...
	if err := resourceData.Set("install_status", "retired"); err != nil {
		return fmt.Errorf("CmdbCiApplHpIndexfailed to set install_status field during destroy action %s", err)
	}
...
//
func copyFromTerraformToServiceNowCmdbCiApplHpIndex(resourceData *schema.ResourceData, ci *CmdbCiApplHpIndexPost) error {
	...
	attrs.Alias = resourceData.Get("alias").(string)
...
//
func copyFromServiceNowToTerraformCmdbCiApplHpIndex(resourceData *schema.ResourceData, jsonData []byte) error {
	ci := CmdbCiApplHpIndexGet{}
	...
	_ = resourceData.Set("alias", attrs.Alias)
