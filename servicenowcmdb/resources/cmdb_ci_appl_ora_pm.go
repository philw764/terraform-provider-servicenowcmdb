...
//
//  Description       :   This file is the resource provider for the cmdb_ci_appl_ora_pm CMDB Class.  This code is executed
//                        when the servicenowcmdb_cmdb_ci_appl_ora_pm keyword is used in a terraform script (*.tf) file
//
//                        This file is will need to be regenerated if the ServiceNow CMDB base CI Class
//                        "cmdb_ci" or of the cmdb_ci_appl_ora_pm CI Class is modified.
///////////////////////////////////////////////////////////////////////////////////////////////////
import (
	"encoding/json"
	"fmt"
...
const CiNamecmdb_ci_appl_ora_pm = "cmdb_ci_appl_ora_pm"
...
// This is essentially the same as CmdbCiApplOraPmGet but does not contain the "value", "link" and
...
//
type CmdbCiApplOraPmPost struct {
	Result struct {
		Attributes struct {
...
// The generateprovidersource utility constructs this STRUCT from metadata pulled from ServiceNow.
//
type CmdbCiApplOraPmGet struct {
	Result struct {
		Attributes struct {
			TcpPort                     string `json:"tcp_port,omitempty"`
			SkipSync                    string `json:"skip_sync,omitempty"`
...
func ResourceCmdbCiApplOraPm() *schema.Resource {
	return &schema.Resource{
		Create: createResourceCmdbCiApplOraPm,
		Read:   readResourceCmdbCiApplOraPm,
		Update: updateResourceCmdbCiApplOraPm,
		Delete: deleteResourceCmdbCiApplOraPm,
...
//
func createResourceCmdbCiApplOraPm(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client) //Client Connection details
	// Use common function to update base attributes
	var ci CmdbCiApplOraPmPost
	if err := copyFromTerraformToServiceNowCmdbCiApplOraPm(resourceData, &ci); err != nil {
		return err
	}
	...
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_appl_ora_pm + "?sysparm_fields=sys_id"
	...
	return readResourceCmdbCiApplOraPm(resourceData, serviceNowClient)
}

//  Read routine - This function is called when Terraform wants to create a new CI in the ServiceNow CMDB
//
func readResourceCmdbCiApplOraPm(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client)
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_appl_ora_pm + "/" + resourceData.Id()
	var jsonData []byte
	...
	if err := copyFromServiceNowToTerraformCmdbCiApplOraPm(resourceData, jsonData); err != nil {
		return err
	}
...
//
func updateResourceCmdbCiApplOraPm(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	servicenowClient := serviceNowClient.(*Client)
...
//
func deleteResourceCmdbCiApplOraPm(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	...
	if err := resourceData.Set("install_status", "retired"); err != nil {
		return fmt.Errorf("CmdbCiApplOraPmfailed to set install_status field during destroy action %s", err)
	}
...
//
func copyFromTerraformToServiceNowCmdbCiApplOraPm(resourceData *schema.ResourceData, ci *CmdbCiApplOraPmPost) error {
...
//
func copyFromServiceNowToTerraformCmdbCiApplOraPm(resourceData *schema.ResourceData, jsonData []byte) error {
	ci := CmdbCiApplOraPmGet{}
