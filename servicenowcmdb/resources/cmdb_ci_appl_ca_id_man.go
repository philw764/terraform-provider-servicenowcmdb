...
//
//  Description       :   This file is the resource provider for the cmdb_ci_appl_ca_id_man CMDB Class.  This code is executed
//                        when the servicenowcmdb_cmdb_ci_appl_ca_id_man keyword is used in a terraform script (*.tf) file
//
//                        This file is will need to be regenerated if the ServiceNow CMDB base CI Class
//                        "cmdb_ci" or of the cmdb_ci_appl_ca_id_man CI Class is modified.
///////////////////////////////////////////////////////////////////////////////////////////////////
import (
	"encoding/json"
	"fmt"
...
const CiNamecmdb_ci_appl_ca_id_man = "cmdb_ci_appl_ca_id_man"
...
// This is essentially the same as CmdbCiApplCaIdManGet but does not contain the "value", "link" and
...
//
type CmdbCiApplCaIdManPost struct {
	Result struct {
		Attributes struct {
...
// The generateprovidersource utility constructs this STRUCT from metadata pulled from ServiceNow.
//
type CmdbCiApplCaIdManGet struct {
	Result struct {
		Attributes struct {
			TcpPort                     string `json:"tcp_port,omitempty"`
			SkipSync                    string `json:"skip_sync,omitempty"`
...
func ResourceCmdbCiApplCaIdMan() *schema.Resource {
	return &schema.Resource{
		Create: createResourceCmdbCiApplCaIdMan,
		Read:   readResourceCmdbCiApplCaIdMan,
		Update: updateResourceCmdbCiApplCaIdMan,
		Delete: deleteResourceCmdbCiApplCaIdMan,
...
//
func createResourceCmdbCiApplCaIdMan(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client) //Client Connection details
	// Use common function to update base attributes
	var ci CmdbCiApplCaIdManPost
	if err := copyFromTerraformToServiceNowCmdbCiApplCaIdMan(resourceData, &ci); err != nil {
		return err
	}
	...
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_appl_ca_id_man + "?sysparm_fields=sys_id"
	...
	return readResourceCmdbCiApplCaIdMan(resourceData, serviceNowClient)
}

//  Read routine - This function is called when Terraform wants to create a new CI in the ServiceNow CMDB
//
func readResourceCmdbCiApplCaIdMan(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

	servicenowClient := serviceNowClient.(*Client)
	SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_appl_ca_id_man + "/" + resourceData.Id()
	var jsonData []byte
	...
	if err := copyFromServiceNowToTerraformCmdbCiApplCaIdMan(resourceData, jsonData); err != nil {
		return err
	}
...
//
func updateResourceCmdbCiApplCaIdMan(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	servicenowClient := serviceNowClient.(*Client)
...
//
func deleteResourceCmdbCiApplCaIdMan(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
	...
	if err := resourceData.Set("install_status", "retired"); err != nil {
		return fmt.Errorf("CmdbCiApplCaIdManfailed to set install_status field during destroy action %s", err)
	}
...
//
func copyFromTerraformToServiceNowCmdbCiApplCaIdMan(resourceData *schema.ResourceData, ci *CmdbCiApplCaIdManPost) error {
...
//
func copyFromServiceNowToTerraformCmdbCiApplCaIdMan(resourceData *schema.ResourceData, jsonData []byte) error {
	ci := CmdbCiApplCaIdManGet{}
