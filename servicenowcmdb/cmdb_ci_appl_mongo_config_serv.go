...
//
//  Description       :   This file is the resource provider for the cmdb_ci_appl_mongo_config_serv CMDB Class.  This code is executed
//                        when the servicenowcmdb_cmdb_ci_appl_mongo_config_serv keyword is used in a terraform script (*.tf) file
//
//                        This file is will need to be regenerated if the ServiceNow CMDB base CI Class
//                        "cmdb_ci" or of the cmdb_ci_appl_mongo_config_serv CI Class is modified.
///////////////////////////////////////////////////////////////////////////////////////////////////
import (
...
const CiNamecmdb_ci_appl_mongo_config_serv  = "cmdb_ci_appl_mongo_config_serv"
...
// This is essentially the same as CmdbCiApplMongoConfigServGet but does not contain the "value", "link" and
...
//
type CmdbCiApplMongoConfigServPost struct {
Result struct {
Attributes   struct {
            ...
            ProcessDirectory    string `json:"process_directory,omitempty"`
...
// The generateprovidersource utility constructs this STRUCT from metadata pulled from ServiceNow.
//
type CmdbCiApplMongoConfigServGet struct {
Result struct {
Attributes   struct {
                TcpPort    string `json:"tcp_port,omitempty"`
                SkipSync    string `json:"skip_sync,omitempty"`
...
func ResourceCmdbCiApplMongoConfigServ() *schema.Resource {
return &schema.Resource{
Create: createResourceCmdbCiApplMongoConfigServ,
Read:   readResourceCmdbCiApplMongoConfigServ,
Update: updateResourceCmdbCiApplMongoConfigServ,
Delete: deleteResourceCmdbCiApplMongoConfigServ,
...
//
func createResourceCmdbCiApplMongoConfigServ(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

servicenowClient := serviceNowClient.(*Client) //Client Connection details
// Use common function to update base attributes
var ci CmdbCiApplMongoConfigServPost
if err := copyFromTerraformToServiceNowCmdbCiApplMongoConfigServ(resourceData, &ci); err != nil {
return err
}
...
SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_appl_mongo_config_serv + "?sysparm_fields=sys_id"
...
return readResourceCmdbCiApplMongoConfigServ(resourceData, serviceNowClient)
}

//  Read routine - This function is called when Terraform wants to create a new CI in the ServiceNow CMDB
//
func readResourceCmdbCiApplMongoConfigServ(resourceData *schema.ResourceData, serviceNowClient interface{}) error {

servicenowClient := serviceNowClient.(*Client)
SnowUrl := CMDBInstanceApi + CiNamecmdb_ci_appl_mongo_config_serv + "/" + resourceData.Id()
var jsonData []byte
...
if err := copyFromServiceNowToTerraformCmdbCiApplMongoConfigServ(resourceData,jsonData);  err != nil {
return err
}
...
//
func updateResourceCmdbCiApplMongoConfigServ(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
servicenowClient := serviceNowClient.(*Client)
...
//
func deleteResourceCmdbCiApplMongoConfigServ(resourceData *schema.ResourceData, serviceNowClient interface{}) error {
...
return fmt.Errorf("CmdbCiApplMongoConfigServfailed to set install_status field during destroy action %s", err)
}
...
//
func copyFromTerraformToServiceNowCmdbCiApplMongoConfigServ (resourceData *schema.ResourceData, ci *CmdbCiApplMongoConfigServPost) error {
...
attrs := &ci.Result.Attributes
                ...
                attrs.ProcessName  = resourceData.Get("process_name").(string)
                ...
                attrs.Port  = resourceData.Get("port").(string)
                ...
                attrs.ProcessDirectory  = resourceData.Get("process_directory").(string)
                ...
                attrs.CommandLine  = resourceData.Get("command_line").(string)
...
//
func copyFromServiceNowToTerraformCmdbCiApplMongoConfigServ (resourceData *schema.ResourceData, jsonData [] byte) error {
ci := CmdbCiApplMongoConfigServGet{}
...
attrs := ci.Result.Attributes
                ...
                _ = resourceData.Set("process_name", attrs.ProcessName)
                ...
                _ = resourceData.Set("port", attrs.Port)
                ...
                _ = resourceData.Set("process_directory", attrs.ProcessDirectory)
                ...
                _ = resourceData.Set("command_line", attrs.CommandLine)
